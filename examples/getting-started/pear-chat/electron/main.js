...
if (!app.isPackaged && rawPearStore) {
  ...
  // In dev, avoid the OS keychain "Safe Storage" prompt. On macOS the real
  // Keychain is bypassed with an in-memory mock; on Linux a plaintext store is
  // used (password-store is a no-op on macOS). Packaged builds keep using the
  // real OS keychain so safeStorage and cookie encryption stay protected.
