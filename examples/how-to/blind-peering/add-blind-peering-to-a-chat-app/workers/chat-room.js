...
class ChatRoom extends ReadyResource {
  ...
  async _open () {
    ...
    this.pairMember = this.pairing.addMember({
      ...
      onadd: async (request) => {
        const inv = await this.view.findOne('@pear-chat-blind-peering/invites', { id: request.inviteId })
        if (!inv) return
  ...
  _setupRouter () {
    this.router.add('@pear-chat-blind-peering/add-writer', async (data, context) => {
      await context.base.addWriter(data.key)
    })
    this.router.add('@pear-chat-blind-peering/add-invite', async (data, context) => {
      await context.view.insert('@pear-chat-blind-peering/invites', data)
    })
    this.router.add('@pear-chat-blind-peering/add-message', async (data, context) => {
      await context.view.insert('@pear-chat-blind-peering/messages', data)
    })
  }
  ...
  async getInvite () {
    const existing = await this.view.findOne('@pear-chat-blind-peering/invites', {})
    if (existing) {
      return z32.encode(existing.invite)
    }
    const { id, invite, publicKey, expires } = BlindPairing.createInvite(this.base.key)
    await this.base.append(
      ChatDispatch.encode('@pear-chat-blind-peering/add-invite', { id, invite, publicKey, expires })
    )
  ...
  async addWriter (key) {
    await this.base.append(
      ChatDispatch.encode('@pear-chat-blind-peering/add-writer', { key: b4a.isBuffer(key) ? key : b4a.from(key) })
    )
  }

  async getMessages ({ reverse = true, limit = 100 } = {}) {
    return await this.view.find('@pear-chat-blind-peering/messages', { reverse, limit }).toArray()
  }

  async addMessage (text, info) {
    const id = Math.random().toString(16).slice(2)
    await this.base.append(
      ChatDispatch.encode('@pear-chat-blind-peering/add-message', { id, text, info })
    )
  }
}
