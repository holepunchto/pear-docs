...
class ChatRoom extends ReadyResource {
  ...
  _setupRouter () {
    ...
    this.router.add('@pear-chat-multi-rooms/add-message', async (data, context) => {
      await context.view.insert('@pear-chat-multi-rooms/messages', data)
    })
  }
  ...
  async getRoomInfo () {
    return await this.view.findOne('@pear-chat-multi-rooms/rooms', {})
  }
  ...
  async addRoomInfo () {
    ...
    await this.base.append(
      ChatDispatch.encode('@pear-chat-multi-rooms/add-room', { id, name: this.name, invite: this.invite, info: this.info })
    )
  }

  async getMessages ({ reverse = true, limit = 100 } = {}) {
    return await this.view.find('@pear-chat-multi-rooms/messages', { reverse, limit }).toArray()
  }

  async addMessage (text, info) {
    const id = Math.random().toString(16).slice(2)
    await this.base.append(
      ChatDispatch.encode('@pear-chat-multi-rooms/add-message', { id, text, info })
    )
  }
}
