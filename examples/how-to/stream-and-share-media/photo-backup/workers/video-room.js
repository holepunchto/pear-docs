...
const createPreviewImage = require('./create-preview-image')
const createPreviewVideo = require('./create-preview-video')
...
class VideoRoom extends ReadyResource {
  ...
  async _open () {
    ...
    this.pairMember = this.pairing.addMember({
      ...
      onadd: async (request) => {
        const inv = await this.view.findOne('@pear-photo-backup/invites', { id: request.inviteId })
        if (!inv) return
  ...
  _setupRouter () {
    this.router.add('@pear-photo-backup/add-writer', async (data, context) => {
      await context.base.addWriter(data.key)
    })
    this.router.add('@pear-photo-backup/add-invite', async (data, context) => {
      await context.view.insert('@pear-photo-backup/invites', data)
    })
    this.router.add('@pear-photo-backup/add-video', async (data, context) => {
      await context.view.insert('@pear-photo-backup/videos', data)
    })
    this.router.add('@pear-photo-backup/add-message', async (data, context) => {
      await context.view.insert('@pear-photo-backup/messages', data)
    })
  }
  ...
  async getInvite () {
    const existing = await this.view.findOne('@pear-photo-backup/invites', {})
    if (existing) {
      return z32.encode(existing.invite)
    }
    const { id, invite, publicKey, expires } = BlindPairing.createInvite(this.base.key)
    await this.base.append(
      VideoDispatch.encode('@pear-photo-backup/add-invite', { id, invite, publicKey, expires })
    )
  ...
  async addWriter (key) {
    await this.base.append(
      VideoDispatch.encode('@pear-photo-backup/add-writer', { key: b4a.isBuffer(key) ? key : b4a.from(key) })
    )
  }

  async getVideos ({ reverse = true, limit = 100 } = {}) {
    const videos = await this.view.find('@pear-photo-backup/videos', { reverse, limit }).toArray()
  ...
  async getMessages ({ reverse = true, limit = 100 } = {}) {
    return await this.view.find('@pear-photo-backup/messages', { reverse, limit }).toArray()
  }

  async addMessage (text, info) {
    const id = Math.random().toString(16).slice(2)
    await this.base.append(
      VideoDispatch.encode('@pear-photo-backup/add-message', { id, text, info })
    )
  }
}
