...
- **Six bare module pages have no pin and no provenance:** `bare-apk`, `bare-channel`, `bare-form-data`, `bare-mime`, `bare-sdl`, `bare-union-bundle`. They carry no `upstreamVersion` and appear in neither `scripts/upstream-commits.json` nor `upstream-commits-state.json`, so there is no record of which upstream version their content describes. Pinning them to npm latest would assert an audit that never happened. The fix is to add them to the commit-watch config and regenerate, which establishes provenance.
