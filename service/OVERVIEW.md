...
- **Answers:** QVAC `completion()` with a selectable model — `QVAC_LLM=quality`
  (Qwen3-4B, the default — the small model fabricates code APIs) or `QVAC_LLM=fast`
  (Llama-3.2-1B). With no LLM available it streams an *extractive* answer instead,
