...
|  **`autostart`**  | Create a linearized view (base.view) immediately                           | Boolean   | `false` |
|    **`apply`**    | Create a linearized view (base.view) immediately using this apply function | Function  | `null`  |
...
| Property         | Description                                                                 | Type       | Default                         |
| ---------------- | --------------------------------------------------------------------------- | ---------- | ------------------------------- |
| **`live`**       | Enable live mode (the stream will continuously yield new nodes)             | Boolean    | `false`                         |
| **`tail`**       | When in live mode, start at the latest clock instead of the earliest        | Boolean    | `false`                         |
| **`map`**        | A sync map function                                                         | Function   | `(node) => node`                |
| **`checkpoint`** | Resume from where a previous read stream left off (`readStream.checkpoint`) | Readstream | `null`                          |
