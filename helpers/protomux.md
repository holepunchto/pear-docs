...
{% embed url="https://github.com/mafintosh/protomux" %}
...
{% hint style="info" %}
`mux.createChannel` returns `null` if the channel should not be opened, it's a duplicate channel or the remote has already closed this one. If you want multiple sessions with the same `protocol` and `id`, set `unique: false` as an option.
{% endhint %}
...
Register a callback to be called every time a new channel is requested.
...
Add/register a message type for a specific encoding. Options include:
