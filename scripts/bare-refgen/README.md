...
A `interface Foo {}` + `declare class Foo { constructor(...) }` merge — Bare's
pattern for every stateful export — puts instance members on the interface but
the constructor on the class. A manifest entry keyed by the bare class name
documents the constructor, so the splicer must land it on `constructor(...)`,
never on the interface (which has no such parameter) — see
[holepunchto/bare-url#20](https://github.com/holepunchto/bare-url/pull/20#discussion_r3622517981)
for what it looks like when this goes wrong.
...
**Loop B — sync upstream docs** (manual only, no workflow wired up). `npm run
emit:ts-doc [-- --only <name>]` regenerates a module's TSDoc (in the `.d.ts`)
*and* its README `## API` section from the model, and commits on `chore/ts-doc`
in `../ts-doc-upstream/<name>`. **It never pushes or opens a PR itself** — that
stays a deliberate, reviewed, per-repo action you take yourself (push to your
own fork, open the PR, read the diff). There is no scheduled or automatic
trigger for this loop, unlike Loop A.
