// scripts/bare-refgen/emit-tsdoc.ts
//
 ...
 *
 *  Bare also uses this same merge for the CONSTRUCTOR: `interface Foo {}` carries
 *  only instance members (it has no construct signature of its own), while
 *  `declare class Foo { constructor(...) }` carries the constructor. A manifest
 *  entry keyed by the bare class name (`Foo`) documents "how to construct a Foo" —
 *  semantically the constructor — so it must land on the `constructor(...)` site,
 *  never on the `interface Foo` site (which merely happens to be declared first in
 *  the file and would otherwise "win" the bare-name key by file order, leaving a
 *  dangling `@param` on a declaration with no such parameter — see holepunchto/
 *  bare-url#20 review comment). `ctorClasses` below identifies every class with an
 *  own constructor so its interface twin never claims the bare-name key, and the
 *  constructor member is given the bare name as an extra alias to claim it instead. */
function injectJsDoc(
  ...
  // Top-level classes that declare their own constructor — the bare-name key
  // belongs to that constructor, not to a same-named interface or namespace.
  const ctorClasses = new Set<string>();
  for (const stmt of sf.statements) {
    if (
      ts.isClassDeclaration(stmt) && stmt.name &&
      stmt.members.some((m) => ts.isConstructorDeclaration(m))
    ) {
      ctorClasses.add(stmt.name.getText(sf));
    }
  }
  ...
  // `bareCtorAlias` (only set when walking the class half of a ctor-bearing pair)
  // lets the constructor member additionally claim the container's own bare-name
  // key, since that's where a "how to construct this" manifest entry is keyed.
  const walkMembers = (
    members: ts.NodeArray<ts.ClassElement | ts.TypeElement>,
    containerPrefix: string,
    bareCtorAlias?: string,
  ) => {
    for (const member of members) {
      if (
        ...
        if (!name) continue;
        const names = qualified(containerPrefix, name);
        if (name === 'constructor' && bareCtorAlias) names.push(bareCtorAlias);
        consider(member, names);
      }
    }
  };
  ...
  const walk = (statements: ts.NodeArray<ts.Statement>, prefix: string) => {
    for (const stmt of statements) {
      if (ts.isModuleDeclaration(stmt) && stmt.body && ts.isModuleBlock(stmt.body)) {
        ...
        if (name) {
          const hasCtorSibling = ctorClasses.has(name);
          // The container's own bare-name key documents its constructor, which
          // for a ctor-bearing class/interface pair lives on the constructor
          // member, not on this declaration site — skip claiming it here so it
          // falls through to the constructor via walkMembers' bareCtorAlias.
          if (!hasCtorSibling) consider(stmt, qualified(prefix, name));
          walkMembers(stmt.members, prefix ? `${prefix}.${name}` : name, hasCtorSibling ? name : undefined);
        }
      } else if (
