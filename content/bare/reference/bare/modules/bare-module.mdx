...
[`bare-module`](https://github.com/holepunchto/bare-module) — Module support for JavaScript. It is a native addon and requires Bare `>=1.32.0`.

<Callout type="warn">
Version 7 is a breaking change from the 6.x line this page previously documented. Loader configuration (`cache`, `protocol`, `conditions`, `builtins`, and a new `concurrency` limit) moved off `Module` instances onto a new [`ModuleLoader`](#moduleloader), which exposes `.import()`/`.link()` (plus `Sync` forms) as the primary entry points. `ModuleProtocol` dropped its `preresolve`/`postresolve`/`addon`/`asset` hooks in favor of a leaner `resolve`/`read`/`exists`/`list` surface, each with a `Sync` counterpart. See the [upstream repo](https://github.com/holepunchto/bare-module) for full migration notes.
</Callout>
...
- `TypeError` — `parentURL` is neither a string nor a WHATWG `URL`, and no `referrer` was given.
...
As `load()`, but synchronous, for a caller that cannot await one. The protocol is driven through its `*Sync` methods and so must answer synchronously. A module with a top-level `await` is returned as soon as its evaluation is started rather than once it has finished, as there is nothing to wait on it here.
...
Resolve the module `specifier` relative to the `parentURL`. `specifier` is a string and `parentURL` is a WHATWG `URL`. `condition` is an optional import condition, defaulting to `'require'`; pass `'asset'` to resolve an asset rather than a module.
...
- `ASSET_NOT_FOUND` — the `'asset'` condition was used and no matching asset could be found.
- `ADDON_NOT_FOUND` — the `'addon'` condition was used and no matching addon could be found.
- `TypeError` — `specifier` is not a string, `parentURL` is not a WHATWG `URL`, or `condition` is not a known condition.

#### `Module.resolveSync(specifier: string, parentURL: URL, opts?: ResolveOptions): URL`

As `resolve()`, but synchronous, for a caller that cannot await one. The protocol is driven through its `*Sync` methods and so must answer synchronously.
...
Centralizes the configuration a graph of modules loads under — `cache`, `protocol`, `conditions`, `builtins`, `resolutions`, and a new `concurrency` limit — instead of it being threaded through every `Module.load()`/`.resolve()` call individually. `.import()` and `.link()` (plus their `Sync` forms) are its primary entry points.

Exposed as `Module.Loader` on the module's default export, or importable directly from the `bare-module/loader` subpath.

#### `new ModuleLoader(opts?: Options)`
...
The cache of loaded modules that the loader was created with, shared with every module of its graph.

#### `concurrency: number`

The maximum number of module reads to perform concurrently while linking. `0` applies no limit.
...
The assumed type of a module without a `type` using an ambiguous extension, such as `.js`. See `Module.constants` for possible values.
...
Exposed as `Module.Protocol` on the module's default export, or importable directly from the `bare-module/protocol` subpath.
...
| Parameter | Type | Default | Description |
| --- | --- | --- | --- |
| `url` | `URL` | — | The directory `URL` to list. |

#### `listSync(url: URL): Iterable<URL>`

Synchronous form of `list()`.
...
**Returns** `boolean` — Whether `value` is a protocol of the same version as this one, whichever copy of `bare-module` created it.
...
Resolve a module `URL` to the `URL` that should be linked. The default implementation returns `url` unchanged.
...
Import attributes instructing how a module should be loaded. A `type` that is not one of the known strings is turned down rather than passed over.
...
Shared loader configuration — the same field set `LoadOptions`, `ResolveOptions`, `CreateRequireOptions`, and `ModuleLoader`'s own constructor options all build on.
...
Options for `Module.load()` / `.loadSync()`.
...
Options for `Module.resolve()` / `.resolveSync()`.
...
Options for `Module.createRequire()`.
...
Options for `ModuleLoader.link()` / `.linkSync()`.
...
Options for `ModuleLoader.import()` / `.importSync()`.
...
The `require.addon` function: imports addon modules, with `host` and `resolve` attached. `host` is the platform and architecture used when resolving addons, following the pattern `<platform>-<arch>[-<environment>]` — the same value as `Bare.Addon.host`.
...
Used to import JavaScript or JSON modules and local files. Relative paths such as `./`, `./foo`, `./bar/baz`, and `../foo` are resolved against the directory named by `__dirname`. POSIX-style paths are resolved in an OS-independent fashion, so the same relative paths work on Windows as they do on POSIX systems.
...
| Parameter | Type | Default | Description |
| --- | --- | --- | --- |
| `specifier` | `string` | — | The module specifier to resolve and load. |
| `options?` | [`RequireOptions`](#requireoptions) | — | Options for the call; `with.type` selects how the file should be loaded — one of `'script'`, `'module'`, `'json'`, `'bundle'`, `'addon'`, `'binary'`, or `'text'` (see [`Module.constants`](#moduleconstants)). A `type` that is not one of these is turned down rather than passed over, as is a `type` that is not a string. |
...
Uses the internal machinery of `require()` to resolve `specifier` relative to the URL `parentURL` and returns the path string.

### `require.addon([specifier][, parentURL])`

Also used to import modules, but loads only addon modules. `specifier` is resolved relative to `parentURL` using the [addon resolution](https://github.com/holepunchto/bare-addon-resolve#algorithm) algorithm.

Returns the exported module contents.

A common pattern for writing an addon module is to use `require.addon()` as the JavaScript module's exports:
...
The platform and architecture used when resolving addons, following the pattern `<platform>-<arch>[-<environment>]`. The same value as `Bare.Addon.host`.

### `require.addon.resolve([specifier][, parentURL])`

Resolves `specifier` relative to `parentURL` as an addon and returns the path string, using the [addon resolution algorithm](https://github.com/holepunchto/bare-addon-resolve#algorithm).

### `require.asset(specifier[, parentURL])`

Resolves `specifier` relative to `parentURL` and returns the path of the asset as a string. Can be used to load assets, for example to load `./foo.txt` from the local file system:
...
The static `import` declaration imports read-only live bindings that are exported by another module. The imported bindings are called _live_ bindings because they are updated by the module that exported the binding, but cannot be re-assigned by the importing module. For more on `import` syntax, see [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/import).
...
A module-relative resolution function that returns the URL string for the module. `specifier` is a string resolved relative to `parentURL`, a WHATWG `URL`.
...
### `import.meta.addon.resolve([specifier][, parentURL])`

Resolves `specifier` relative to `parentURL` as an addon and returns the URL string, using the [addon resolution algorithm](https://github.com/holepunchto/bare-addon-resolve#algorithm).

### `import.meta.asset(specifier[, parentURL])`

Resolves `specifier` relative to `parentURL` and returns the URL of the asset as a string.
