...
[`bare-url`](https://github.com/holepunchto/bare-url) — WHATWG URL implementation for JavaScript, built on [`liburl`](https://github.com/holepunchto/liburl). It is a native addon.
...
| --- | --- | --- | --- |
| `input` | `string` \| `URL` | — | The URL string to parse. |
...
- `INVALID_FILE_URL_HOST` — (non-Windows) the URL has a host other than empty or
`'localhost'`.
- `INVALID_FILE_URL_PATH` — the URL path contains an encoded path-separator or NUL
