...
The protocol an object must implement to be passed to `pipe.accept(target)` or received via `pipe.write(chunk, handle, ...)`; see [`IPCAcceptable` protocol](#ipcacceptable-protocol).
