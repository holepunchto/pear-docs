...
upstreamVersion: "1.6.7"
---

<mark style={{ backgroundColor: '#7dde9a', padding: '2px 8px', borderRadius: '4px', fontSize: '0.85em', fontWeight: 500 }}>stable</mark>

[`bare-module-lexer`](https://github.com/holepunchto/bare-module-lexer) — Heuristic lexer for detecting imports and exports in JavaScript modules. It is a native addon and requires Bare `>=1.0.0`.
...
An import detected in the source. `type` is a combination of the `lex.constants` flags (for example `REQUIRE`, `IMPORT`, `DYNAMIC`); `position` holds the offsets `[importStart, specifierStart,
specifierEnd]`.
