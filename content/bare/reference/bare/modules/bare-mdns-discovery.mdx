...
description: "Minimal mDNS/DNS-SD service discovery for Bare"
docType: reference
product: bare
schemaType: APIReference
upstreamVersion: "0.2.3"
---

<mark style={{ backgroundColor: '#8484ff', padding: '2px 8px', borderRadius: '4px', fontSize: '0.85em', fontWeight: 500 }}>experimental</mark>

[`bare-mdns-discovery`](https://github.com/holepunchto/bare-mdns-discovery) — Minimal mDNS/DNS-SD service discovery.

<Callout type="warn">
Upstream marks this module experimental: the README describes it as a "Claude generated prototype." Expect the API and behavior to change.
</Callout>
...
const services = await discovery.discover({ timeout: 10_000 }) // timeout in ms (10 seconds)
...
Whether internal activity is logged to the console (default `false`).
...
- Builds on `bare-dgram`.
- For discovering nearby peers without a shared network (Bluetooth LE instead of mDNS over WiFi), see [nearby-discovery](https://github.com/holepunchto/nearby-discovery). Not yet documented on this site.
