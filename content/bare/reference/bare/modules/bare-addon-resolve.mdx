...
| `parentURL` | `URL` | — | The URL to resolve `specifier` relative to. |
| `opts?` | [`ResolveOptions`](#resolveoptions) | — | Options; see [`ResolveOptions`](#resolveoptions). If `opts.resolutions` is set, it is checked first and a matching entry short-circuits the rest of the algorithm. |
