...
| `opts?` | [`StowOptions`](#stowoptions) | — | Options; `client`/`server` wire an RPC library into the harness/shim, `base` sets the module graph root (defaults to the directory containing `entry`), `hosts` restricts the build to a subset of the target's supported host triples (defaults to all of them), `resolveTarget`/`resolveRPC` resolve a target/RPC name that isn't in the built-in registry, and any other option is forwarded to `bare-pack`. |
...
- The CLI is installed globally via `npm i -g bare-stow`.
