...
[`bare-tls`](https://github.com/holepunchto/bare-tls) — Transport Layer Security (TLS) streams for JavaScript, built on [BoringSSL](https://boringssl.googlesource.com/boringssl/). It is a native addon and requires Bare `>=1.7.0`.

Mirrors the Node.js [`tls`](https://nodejs.org/api/tls.html) module. Bundles Mozilla's root certificates, so certificate verification works out of the box without passing a `ca` option.
...
| `opts?` | [`TLSSocketOptions`](#tlssocketoptions) | — | Options; `rejectUnauthorized`, `eagerOpen`, and `allowHalfOpen` default to `true`, `readBufferSize` to `65536`, and `ca` to the bundled Mozilla root certificates. |
...
```ts
connect(opts: TLSSocketOptions & { port: number; host?: string }, onconnect?: () => void): TLSSocket
```

Alias of `createConnection()`, for Node.js compatibility.
