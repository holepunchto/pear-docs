...
| `options?` | `any` | — | Extra options, merged directly into the underlying `http`/`https` request — for example `timeout` or a keep-alive `agent`. `requireJobName` (default `true`) controls whether `push()`/`pushAdd()`/`delete()` require a `jobName`; set it to `false` for gateways that don't group by job name, such as the [Gravel Gateway](https://github.com/sinkingpoint/prometheus-gravel-gateway). |
...
The shape of `inc()`'s single argument on a `Counter` created with `enableExemplars: true`. The registry the metric is collected from must be set to the OpenMetrics content type (via `setContentType()`) for the exemplar to be exported.
...
The shape of `observe()`'s single argument on a `Histogram` created with `enableExemplars: true`. The registry the metric is collected from must be set to the OpenMetrics content type (via `setContentType()`) for the exemplar to be exported.
...
`percentiles` defaults to `[0.01, 0.05, 0.5, 0.9, 0.95, 0.99, 0.999]`. Setting `maxAgeSeconds` and `ageBuckets` enables a sliding window: `maxAgeSeconds` is how old a bucket can be before it's reset, and `ageBuckets` is how many buckets make up the window. When `pruneAgedBuckets` is `false` (the default), the metric value stays present even when empty (with percentile values of `0`); set it to `true` to omit the metric from exports while it's empty.
...
Options for `collectDefaultMetrics()`. `register` defaults to the global default registry. `prefix` defaults to no prefix. `gcDurationBuckets` sets custom buckets for the GC-duration histogram, defaulting to `[0.001, 0.01, 0.1, 1, 2, 5]` (seconds). `eventLoopMonitoringPrecision` is the event-loop-lag sampling rate in milliseconds (must be greater than `0`), defaulting to `10`. `labels` applies a fixed set of labels to every default metric — useful in a clustered environment.
