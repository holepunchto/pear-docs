...
[`bare-type-stripper`](https://github.com/holepunchto/bare-type-stripper) — Heuristic lexer for stripping TypeScript type syntax to produce plain JavaScript. It is a native addon and requires Bare `>=1.0.0`.
...
- `SyntaxError` — the source contains non-erasable TypeScript syntax (`enum`/`const enum`,
