...
A **blind peer** is a dedicated replicator that stores and serves [Hypercores](/bare/reference/building-blocks/hypercore) **without decrypting or interpreting** their contents. Blind peering is one way of **putting the server in serverless**: always-on reachability like a hosted service, but the operator never holds the plaintext. It joins swarms on request and caches blocks so other peers can find a source. Blind peering separates "make this core findable" from "I personally hold the plaintext."
...
Wire the client to your app's existing [Hyperswarm](/bare/reference/building-blocks/hyperswarm) and [Corestore](/bare/reference/helpers/corestore):
...
- [Keep data available with blind peering](/bare/how-to/blind-peering/keep-data-available-with-blind-peering)—step-by-step walkthrough of the client library above.
- [Add blind peering to a chat app](/bare/how-to/blind-peering/add-blind-peering-to-a-chat-app)—the same client layered onto a chat app worker.
