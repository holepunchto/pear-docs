...
Everything else is left to userland. There is no built-in `fs`, no built-in `http`, no bundled standard library—those live in installable [`bare-*` modules](/bare/reference/bare/modules) you add only when you need them. The runtime itself stays succinct and, well, *bare*.
...
No. Bare is a separate runtime built on `libjs` and `libuv`. The surface looks familiar—asynchronous, event-driven, module-based—but Bare drops Node's server-oriented assumptions and its bundled standard library. The [`bare-node`](https://github.com/holepunchto/bare-node) shim maps many Node.js builtins onto their `bare-*` equivalents to ease porting. For what that means in practice for an existing app, see [Migrating from Node.js](/bare/explanation/migrating-from-nodejs).

### Can I use npm modules with Bare?

Pure-JavaScript packages from npm generally work, and Bare uses the npm dependency model. Packages that depend on Node's built-in modules need the corresponding [`bare-*` module](/bare/reference/bare/modules) (or the `bare-node` shim), and packages with native addons need to be built against Bare's addon API rather than Node's N-API (the [`bare-compat-napi`](https://github.com/holepunchto/bare-compat-napi) headers ease that transition).

<Callout type="info">
For the full builtin-to-`bare-*` mapping, see [Node.js compatibility](/bare/reference/bare/modules#nodejs-compatibility).
</Callout>

### Do users need Node.js installed to run a Bare app?

No. A Bare program can be compiled into a single standalone executable with `bare-build`—this is how the [`hello-pear-bare`](/p2p/getting-started/from-a-template/start-from-hello-pear-bare) template ships—so no Node.js, Bare, or [Pear CLI](/pear/reference/pear/cli) is required on the user's machine.
...
- [Bare modules](/bare/reference/bare/modules)—the `bare-*` standard library you opt into.
...
- [How the stack fits together](/p2p/explanation/how-the-stack-fits-together)—where the runtime sits relative to Pear.
