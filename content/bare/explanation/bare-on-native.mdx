...
* The **core** runs as a Bare *worklet*—an isolated Bare thread—and owns everything peer-to-peer:
   ...
   * opens the [Corestore](/p2p/reference/helpers/corestore)
   * runs [Hyperswarm](/p2p/reference/building-blocks/hyperswarm) and [HyperDHT](/p2p/reference/building-blocks/hyperdht)
...
[`bare-kit`](/bare/reference/bare/bare-kit) is the toolkit that runs a Bare core inside a native app. It exposes a web-worker-like API for starting and managing isolated Bare threads—called *worklets*—each with an IPC channel to the host.
...
* from React Native or Expo, [`react-native-bare-kit`](/bare/reference/bare/bare-kit) gives you the same `Worklet` and `IPC` objects in JavaScript;
* The worklet honours the Bare [lifecycle](/bare/explanation/bare-runtime#the-lifecycle), so the host can suspend and resume the core in step with the OS's app-lifecycle rules.

Because the worklet is just Bare, the core you ship to mobile is the *same JavaScript* you'd run in a desktop [worker](/pear/explanation/workers) or a standalone terminal binary. Write it once; embed it everywhere.
...
* [`hyperschema`](https://github.com/holepunchto/hyperschema) defines versioned, append-only data structures and generates [`compact-encoding`](/p2p/reference/helpers/compact-encoding) codecs for them.
* [`bare-rpc`](/bare/reference/modules/bare-modules) frames requests and replies over a duplex stream, with a unique command number per method.
...
This is the architecture behind [Keet's](https://keet.io) identical behaviour on phones, laptops, and terminals: one core, several UIs. The desktop ([Electron](/pear/explanation/pear-desktop-architecture)) and terminal ([standalone Bare binary](/p2p/getting-started/from-a-template/start-from-hello-pear-bare)) hosts ship today. On mobile, [`pear-runtime`](/pear/reference/pear/runtime) itself targets desktop; its mobile counterpart is [`pear-mobile`](https://www.npmjs.com/package/pear-mobile), the embeddable Pear runtime for mobile applications—or wire the worklet and updates yourself with `bare-kit`.

## Common questions

### Does Bare run on iOS and Android?

Yes. `bare-kit` embeds a Bare worklet in native iOS and Android apps today, and `react-native-bare-kit` does the same for React Native and Expo. The [`pear-runtime`](/pear/reference/pear/runtime) equivalent for mobile is [`pear-mobile`](https://www.npmjs.com/package/pear-mobile) (noted earlier on this page).
...
- [`bare-kit` reference](/bare/reference/bare/bare-kit)—the Worklet and IPC API for iOS, Android, and React Native.
- [Embed Bare in a React Native app](/bare/how-to/run-on-native/embed-bare-in-react-native)—start a worklet and exchange messages.
- [Type a native RPC bridge](/bare/how-to/run-on-native/type-a-native-rpc-bridge)—generate a typed seam with `hyperschema` and `bare-rpc`.
- [Bundle a Bare app](/bare/how-to/run-on-native/bundle-a-bare-app)—produce an embeddable bundle or a standalone binary.
- [Workers](/pear/explanation/workers)—the same host-and-core split on the desktop.
- [Inside Bare](/bare/explanation/bare-runtime)—the runtime the worklet is an instance of.
