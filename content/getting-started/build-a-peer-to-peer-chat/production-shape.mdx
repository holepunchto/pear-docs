...
This is **part 2 of 4** in the [getting started path](/getting-started). [Part 1](/getting-started/build-a-peer-to-peer-chat/build-a-peer-to-peer-chat) built a working chat from scratch with the simplest form of the runtime ([`PearRuntime.run`](/reference/runtime#running-workers)). This part reshapes it into the **hello-pear-electron production scaffold** — the exact shape Pear's official [`hello-pear-electron`](https://github.com/holepunchto/hello-pear-electron) template ships, and the same shape [Keet](https://keet.io) and [PearPass](https://pass.pears.com) use under the hood.
...
- Embeds the [`pear-runtime`](/reference/runtime) OTA updater in its **own** Bare worker, so update traffic never blocks the chat.
...
- The working chat from [part 1 — build the peer-to-peer chat](/getting-started/build-a-peer-to-peer-chat/build-a-peer-to-peer-chat), so you are comfortable with [`PearRuntime.run()`](/reference/runtime#running-workers), [Bare workers](/explanation/workers), and [Hyperswarm](/reference/building-blocks/hyperswarm) topics.
...
cd pear-docs/examples/getting-started/production-shape
npm install
npm run build
```
...
You should now have a `production-shape/` directory laid out like this:
...
```js file=<rootDir>/examples/getting-started/production-shape/workers/index.js title="workers/index.js" lineNumbers {9-13,15-16,23-24,26-27,29-30,32-34,37-40}
```
...
- **Lines 23–24:** the worker wraps [`Bare.IPC`](/reference/runtime#running-workers) in a [`framed-stream`](https://www.npmjs.com/package/framed-stream), then pauses it until the task is ready. From here on, worker and renderer talk by exchanging JSON over this framed pipe — no typed RPC layer.
...
```js file=<rootDir>/examples/getting-started/production-shape/workers/worker-task.js title="workers/worker-task.js" lineNumbers {18-28,42-49,58-61,63-67}
```
...
```js file=<rootDir>/examples/getting-started/production-shape/workers/chat-room.js#L127-L137 title="workers/chat-room.js"
```
...
```js file=<rootDir>/examples/getting-started/production-shape/workers/chat-room.js#L37-L47 title="workers/chat-room.js"
```
...
```js file=<rootDir>/examples/getting-started/production-shape/workers/chat-room.js#L72-L85 title="workers/chat-room.js"
```
...
```js file=<rootDir>/examples/getting-started/production-shape/workers/chat-room.js#L52-L68 title="workers/chat-room.js"
```
...
```js file=<rootDir>/examples/getting-started/production-shape/schema.js#L9-L34 title="schema.js — records"
```

`spec/db/` — typed HyperDB collections (`@pear-chat/messages`, `@pear-chat/invites`), each keyed by `id`:

```js file=<rootDir>/examples/getting-started/production-shape/schema.js#L36-L48 title="schema.js — collections"
```

`spec/dispatch/` — typed Autobase append-payload encoders, one per record type:

```js file=<rootDir>/examples/getting-started/production-shape/schema.js#L50-L55 title="schema.js — dispatch"
```
...
```js file=<rootDir>/examples/getting-started/production-shape/electron/main.js#L101-L138 title="electron/main.js — chat worker"
```
...
```js file=<rootDir>/examples/getting-started/production-shape/electron/main.js#L142-L179 title="electron/main.js — updater worker"
```

`applyUpdate` swaps the app on disk; `app:afterUpdate` relaunches the process. The renderer wires both into a button:

```js file=<rootDir>/examples/getting-started/production-shape/electron/main.js#L215-L247 title="electron/main.js — apply + relaunch"
```
...
```js file=<rootDir>/examples/getting-started/production-shape/electron/main.js#L253-L292 title="electron/main.js — single instance"
```
...
```js file=<rootDir>/examples/getting-started/production-shape/electron/preload.js title="electron/preload.js" {8,16,27-39}
```

This is the **single door** between the HTML renderer and the Bare workers. It is intentionally narrow — `startWorker`, `writeWorkerIPC`, `onWorkerIPC`, `onWorkerExit` for the worker, `onPearEvent` / `applyUpdate` / `appAfterUpdate` for OTA, and one extra this part adds: **`writeClipboard`** (line 8), which the copy-invite button calls. The renderer sends `{ type: 'add-message', text }` and receives `{ type: 'messages' }`, `{ type: 'peers' }`, and `{ type: 'invite' }` — all plain JSON over the bridge.
...
The static chat shell: a header with the Pear logo, a peer-status dot, a **Copy invite** button, an "Update ready" button (hidden until OTA fires), a scrollable message list, and a composer form. Every class is a Tailwind utility scanned out of the markup at build time:

```html file=<rootDir>/examples/getting-started/production-shape/renderer/index.html#L12-L47 title="renderer/index.html — header"
```
...
```js file=<rootDir>/examples/getting-started/production-shape/renderer/app.js#L90-L105 title="renderer/app.js — worker IPC"
```

The copy button writes the cached invite to the clipboard through the bridge:

```js file=<rootDir>/examples/getting-started/production-shape/renderer/app.js#L73-L78 title="renderer/app.js — copy invite"
```
...
```js file=<rootDir>/examples/getting-started/production-shape/renderer/app.js#L80-L88 title="renderer/app.js — OTA banner"
```
...
In a second terminal, still in `production-shape/`:

```bash skip="desktop-gui"
npm start -- --storage /tmp/pear-chat-user2 --name user2 --invite <paste-invite>
```

A second window opens. Within a few seconds the two peers pair via [`blind-pairing`](https://www.npmjs.com/package/blind-pairing), the second becomes a writer on the shared Autobase, and both windows show the message history and tick the peer dot to green. Type in one window, hit Enter, and watch it appear in the other.
...
| Updater | `workers/main.js` (PearRuntime OTA in Bare) | [Runtime](/reference/runtime) |
...
- [Add blind peering to a chat app](/how-to/store-and-replicate/add-blind-peering-to-a-chat-app) — keep the room online when its writers are offline.
