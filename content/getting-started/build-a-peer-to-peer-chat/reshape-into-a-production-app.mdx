...
- **Lines 23–24:** the worker wraps [`Bare.IPC`](/reference/pear/runtime#running-workers) in a [`framed-stream`](https://www.npmjs.com/package/framed-stream), then pauses it until the task is ready. From here on, worker and renderer talk by exchanging JSON over this framed pipe. This tutorial keeps that layer deliberately simple to stay focused on the worker lifecycle; for a real app, [HRPC](/reference/helpers/hrpc) is the recommended way to replace hand-written `{ type: '...' }` messages with generated, typed request/response methods—see [Structured RPC over IPC](/explanation/workers#structured-rpc-over-ipc).
