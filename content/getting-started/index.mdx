...
- Part 2 swaps the static `PearRuntime.run()` boot helper from part 1 for a full `new PearRuntime({ ... })` instance, then keeps using that instance's `pear.run(...)` method to spawn the worker. It also plumbs OTA events through the bridge and gives the worker a [Corestore](/reference/helpers/corestore) so messages survive a restart.
