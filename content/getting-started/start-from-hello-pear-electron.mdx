...
Take this path when you want a production-shaped Electron app from the first commit. If you would rather learn the moving parts by building up from scratch, follow the [four-part path](/getting-started) instead; if you want a scaffold from a `pear://` link, see [`pear init`](/reference/pear/cli#pear-init).
