...
description: "Fixes for Pear and Bare development issues."
---
...
The `Pear.teardown(cb)` callback is triggered whenever the Pear app start to unload. If it is not exiting, then something is keeping the applications event loop running. A common cause of this is not cleaning up the [worker pipe](./api.md#pear-worker) by calling `pipe.end()` to gracefully end the writable part of the stream.
