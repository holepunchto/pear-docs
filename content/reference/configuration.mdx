...
By default, [`pear run`](/reference/cli#pear-run) considers link pathnames to be entrypoints. This means `pear run` can execute any valid file staged to a Pear application. For example `pear run pear://<key>/some/path.js` would run some/path.js if it's valid. In that case [`Pear.app.route`](/reference/api#pear-app-route) would contain `/some/path.js` and [`Pear.app.entrypoint`](/reference/api#pear-app-entrypoint) would also contain `/some/path.js`.
...
In this case `pear run pear://<key>/some/path.js` would run the application entrypoint, [`Pear.app.route`](/reference/api#pear-app-route) would contain `/some/path.js` while [`Pear.app.entrypoint`](/reference/api#pear-app-entrypoint) would also be `/` - because `Pear.app.route` holds the raw pathname whereas `Pear.app.entrypoint` holds the mapped pathname.
...
Assets are automatically stored in the platform folder. Use [`Pear.app.assets[namespace].path`](/reference/api#pear-app-assets) within the application to get the path to the asset. For example, `Pear.assets.ui.path`.
...
`links` can be an object or an array. If it's an object, naming the key makes it easy to reference from [`Pear.config.links`](/reference/api#pear-config)
