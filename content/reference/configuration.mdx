...
The `package.json` is used by `pear` and [`pear-runtime`](/reference/runtime) for application builds, delivery and updates.
...
`pear run` is deprecated; the fields below relate to it. Use the [`pear-runtime`](/reference/runtime) module instead.
...
By default, [`pear run`](/reference/cli#pear-run-flags-linkdir-app-args) considers link pathnames to be entrypoints. This means `pear run` can execute any valid file staged to a Pear application. For example `pear run pear://<key>/some/path.js` would run some/path.js if it's valid. In that case [`Pear.app.route`](/reference/api#pearapproute-string) would contain `/some/path.js` and [`Pear.app.entrypoint`](/reference/api#pearappentrypoint-string) would also contain `/some/path.js`.
...
Assets are automatically stored in the platform folder. Use [`Pear.app.assets[namespace].path`](/reference/api#pearappassets--string) within the application to get the path to the asset. For example, `Pear.assets.ui.path`.
...
`links` can be an object or an array. If it's an object, naming the key makes it easy to reference from [`Pear.config.links`](/reference/api#pearconfig-object)
...
- [Pear CLI](/reference/cli) — the commands that read these fields (`pear stage`, `pear seed`, `pear release`).
- [Pear API](/reference/api) — `Pear.config` and `Pear.app.*` surface configuration values at runtime.
- [`pear-runtime` reference](/reference/runtime) — embedding the runtime that consumes `package.json` and these fields.
