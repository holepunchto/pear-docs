...
[src](https://github.com/holepunchto/compact-encoding/blob/v3.3.0/index.js#L1036)
...
[src](https://github.com/holepunchto/compact-encoding/blob/v3.3.0/index.js#L1044)
...
| Codec | Notes |
| --- | --- |
| `cenc.buffer` | Buffer with a `uint` length prefix. Decoding an empty buffer returns `null`. |
...
**Strings** — each is length-prefixed and also exposes a `.fixed(n)` factory for a fixed-length variant (e.g. `cenc.hex.fixed(32)`).
