...
`Autobee` gives a [Hyperbee](/reference/building-blocks/hyperbee) many writers. Each peer appends to its own local [Hypercore](/reference/building-blocks/hypercore); Autobee linearizes those cores into one causal order and replays it through your `apply` handler, which writes into a shared B-tree view. Every peer that replicates the same nodes derives the same view.

It sits alongside [Autobase](/reference/building-blocks/autobase) rather than replacing it. Autobase linearizes into a view of any shape you open; Autobee is the case where that view is always a Hyperbee, with the B-tree write path built in.
...
| --- | --- | --- | --- |
| `store` | [`Corestore`](/reference/helpers/corestore) | — | The Corestore holding the system, writer, and view cores. |
...
The readable view, updated after each apply cycle. Without an `open` handler this is the Hyperbee snapshot itself, so read it with the [Hyperbee](/reference/building-blocks/hyperbee) API — `view.get(key)`, `view.createReadStream()`, `view.peek(range)`.
...
Create a replication stream. Arguments match [`store.replicate()`](/reference/helpers/corestore), and the stream is also registered with the wakeup protocol.
...
- [Build a multiwriter database with Autobee](/how-to/store-and-replicate/build-a-multiwriter-database-with-autobee)—two-peer walkthrough covering `apply`, the `update` hook, and granting write access.
- [Autobase](/reference/building-blocks/autobase)—the general multiwriter linearizer, for views that are not a Hyperbee.
- [Hyperbee](/reference/building-blocks/hyperbee)—the single-writer B-tree whose read API `db.view` exposes.
- [Corestore](/reference/helpers/corestore)—storage and replication manager for the system, writer, and view cores.
- [Hypercore](/reference/building-blocks/hypercore)—append-only log primitive that each writer appends to.
