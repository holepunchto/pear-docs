...
**Deprecated.** The `global.Pear` direct-access API documented here is superseded by the [`pear-runtime`](/reference/runtime) module — instantiate `Pear` and use its API instead. This page is retained for reference and for the still-current [`global.Bare`](#globalbare) APIs.
</Callout>

Pear is built on [Bare](/reference/bare-modules). Pear applications have a `global.Pear` object and a`global.Bare` object. For background on the runtime split, see [Runtime and languages](/explanation/runtime-and-languages); for the surrounding `pear-*` library catalog, see [Modules](/reference/modules).
...
The majority of capabilities are supplied via [Pear Modules](/reference/modules).
...
The value of the [`pear run --checkout`](/reference/cli#pear-run-flags-linkdir-app-args) flag. Same as [`Pear.app.flags.checkout`](#pear-app-checkout).

**References**

* [`pear run`](/reference/cli#pear-run-flags-linkdir-app-args)
* [`Pear.app.flags`](#pearappflags-object)
...
* [`pear release`](/reference/cli#pear-release-link-dir)
...
**References**
* [`pear.links`](/reference/configuration#pearlinks-objectarray)
* [`pear run`](/reference/cli#pear-run-flags-linkdir-app-args)
...
The configuration provided [`pear.routes`](/reference/configuration#pearroutes-objectstring) mapping object.
...
The link pathname (`pear://<key>/<pathname>`), after any route mappings have been applied per [`pear.routes`](/reference/configuration#pearroutes-objectstring).

Includes the leading `/`, e.g given `pear://foo/bar/baz`, `Pear.app.entrypoint` would be `/bar/baz`.

Only `Pear.app.entrypoint` supports route-mapping via the [`pear.routes`](/reference/configuration#pearroutes-objectstring) mapping object. `Pear.app.route` is provides access the raw pathname, including the leading slash (`/`), while `Pear.app.linkData` is legacy, excludes the slash, but is still supported.

* [`Pear.app.route`](#pearappentrypoint-string)
* [`Pear.app.routes`](#pearapproutes-string)
* [`pear.routes`](/reference/configuration#pearroutes-objectstring)
...
* [`Pear.app.entrypoint`](#pearappentrypoint-string)
* [`Pear.app.routes`](#pearapproutes-string)
* [`pear.routes`](/reference/configuration#pearroutes-objectstring)
...
* [`Pear.app.entrypoint`](#pearappentrypoint-string)
* [`Pear.app.route`](#pearapproutes-string)
* [`Pear.app.link`](#pearapplinks-objectarray)
* [`pear run`](/reference/cli#pear-run-flags-linkdir-app-args)
...
Per [`pear-assets`](/reference/configuration#pear-assets) configuration assets are fetched and stored to disk. Use `Pear.app.assets[namespace].path` to get the path that given asset is stored to.
...
Whether the current application is a [`pre`](/reference/configuration#pear-pre) script.
...
Compat-mode opt-in for legacy applications. For JavaScript projects, use the [`pear-runtime` reference](/reference/runtime).
...
**DEPRECATED** Use [`Pear.app`](#pearapp-object).
...
**DEPRECATED** No option. Do not use.
...
The majority of capabilities are supplied via [Bare modules](/reference/bare-modules).
...
- [`pear-runtime` reference](/reference/runtime) — the supported, non-deprecated way to access these capabilities from a Pear app.
- [Configuration](/reference/configuration) — the `package.json` fields read at the values surfaced in `Pear.config` and `Pear.app.*`.
- [Pear CLI](/reference/cli) — commands that drive the lifecycle these APIs reflect (`pear stage`, `pear release`, `pear seed`).
- [Modules](/reference/modules) — the `pear-*` libraries that supply most application capabilities.
- [Bare modules](/reference/bare-modules) — runtime primitives the API builds on.
