...
Removed in Pear v3: it fed the `pear stage` JavaScript static-analysis / warmup phase (used mainly with the removed `pear stage --compact` flag), and `pear stage` no longer reads it. To control what is staged in v3, use [`pear.stage.ignore`](#pear-stage-ignore) (or the `pear.stage.only` filter).
