...
That diff is the complete CLI surface change. Mark anything that differs from a previous release rather than describing it in prose (src/components/version). `Since` = added in that release, `Until` = removed in it. Three tools, by the size of what changed:
...
Add the release to DOCS_VERSIONS in src/lib/docs-versions.ts when it changes the documented surface, or it will not appear in the dropdown. Pass `label` to override badge text. */}
