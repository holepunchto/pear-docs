...
That diff is the complete CLI surface change. Mark anything that differs from a previous release with the <Since v="x.y.z" /> / <Until v="x.y.z" /> components (src/components/version) rather than prose — they render a badge today and are what the version dropdown filters on. `Since` = added in that release, `Until` = removed in it; pass `label` to override the badge text.
