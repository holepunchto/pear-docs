...
**Deprecated.** The `global.Pear` direct-access API documented here is superseded by the [`pear-runtime`](/reference/pear/runtime) module — instantiate `Pear` and use its API instead. This page is retained for reference and for the still-current [`global.Bare`](#globalbare) APIs.
</Callout>
...
**DEPRECATED** — superseded by the [`pear-runtime`](/reference/pear/runtime) API (instantiate `Pear` and use its instance instead). Applies to all `Pear.app.*` properties below.
...
**DEPRECATED** — use [`Bare.argv`](#bare-argv).
...
**DEPRECATED** — use [`Bare.pid`](#bare-pid).
...
**DEPRECATED** — use [`Bare.exitCode`](#bare-exitcode).
...
**DEPRECATED** — use [`Bare.exit()`](#bare-exit).
...
- [Pear CLI](/reference/pear/cli) — commands that drive the lifecycle these APIs reflect (`pear stage`, `pear release`, `pear seed`).
