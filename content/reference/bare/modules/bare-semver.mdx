...
Parse a range string into a `Range` instance. Supports:
- comparison operators (`<`, `<=`, `>`, `>=`, `=`), for example `>=1.0.0 <2.0.0`
- caret ranges (`^1.2.3`)
- tilde ranges (`~1.2.3` or `~>1.2.3`)
- X-ranges and wildcards (`1.2.x`, `1.*`, `*`)
- hyphen ranges (`1.2.3 - 2.3.4`)
- partial versions (`1.2`, `1`)
- logical OR (`||`), for example `1.2.3 || >=2.0.0`
