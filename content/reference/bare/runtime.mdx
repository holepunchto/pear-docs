...
As of 2026-08-06, the generated delta across the tracked window (1.23.5 -> 1.31.0) has
no marker-worthy change to what's documented on THIS page: `Bare.Addon`/`Bare.Thread`
becoming top-level siblings in 1.31.0 is a .d.ts authoring change only — both remain
reachable as `Bare.Addon`/`Bare.Thread` at runtime via the namespace merge, so it is not
a real surface change. The `Bare.versions` index-signature parameter rename (`package` ->
`library`, 1.28.2) is a type-only cosmetic change. `Bare.exiting`/`Bare.suspended`/
