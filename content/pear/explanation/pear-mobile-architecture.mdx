...
Activation timing also differs by platform, for a structural reason: neither iOS nor Android gives an app a way to hot-swap its own binary path the way a desktop shell renames a directory and relaunches. iOS re-reads the bundle URL on every JS reload; Android caches it when the React host is first created, so activation waits for a full process restart. See [Runtime activation](/pear/reference/pear/mobile-boot-control#runtime-activation) for exactly when each platform picks up a staged update.
...
- [Troubleshoot common issues](/pear/how-to/troubleshooting)—general Pear troubleshooting; mobile-specific issues live in [Deploy your mobile application](/pear/how-to/operate-an-app/manual-deployment/mobile#troubleshooting).
