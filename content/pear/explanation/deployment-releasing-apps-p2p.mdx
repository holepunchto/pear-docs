...
Read it before you run commands so the mental model matches the CLI (concrete steps live under [Deploy your application](/pear/how-to/operate-an-app/manual-deployment/deployment) and [Build desktop distributables](/pear/how-to/operate-an-app/build-and-package/build-desktop-distributables)).

<include>../../_snippets/_ci-publish-shortcut-callout.mdx</include>

<Callout type="info">
**Operator detail:** [Deploy your application](/pear/how-to/operate-an-app/manual-deployment/deployment), [Build desktop distributables](/pear/how-to/operate-an-app/build-and-package/build-desktop-distributables), and [Troubleshoot desktop releases](/pear/how-to/operate-an-app/manual-deployment/troubleshoot-desktop-releases) carry the same pipeline as these diagrams. The [Glossary](#glossary) below defines the terms.

</Callout>

<Callout type="info">
Mobile reuses this same stage → provision → multisig model over a different payload shape—a single cross-platform JS bundle instead of per-OS distributables. See [Pear mobile application architecture](/pear/explanation/pear-mobile-architecture#updates) and [Deploy your mobile application](/pear/how-to/operate-an-app/manual-deployment/mobile).
</Callout>
...
A running Pear desktop app polls the [Hyperdrive](/p2p/reference/building-blocks/hyperdrive) behind its `upgrade` link, downloads new application data when the [Hypercore](/p2p/reference/building-blocks/hypercore) length advances, and emits two events from [`pear.updater`](/pear/reference/pear/runtime). The host process forwards them to renderers so the UI can react:
...
- [`updating`](/pear/reference/pear/runtime#updates) fires when the updater begins streaming new blocks; treat it as "background work in progress."
- [`updated`](/pear/reference/pear/runtime#updates) fires when the new build is fully on disk; it is safe to swap.
...
The two-event split is what lets you build UI like the "Update ready" banner in [Reshape into a production app](/p2p/getting-started/build-a-peer-to-peer-chat/reshape-into-a-production-app): show a spinner on `updating`, swap to a restart button on `updated`, restart the app on click.
...
The diagrams below follow the same Foundational Steps as [Deploy your application](/pear/how-to/operate-an-app/manual-deployment/deployment)—the flow behind [`hello-pear-electron`](https://github.com/holepunchto/hello-pear-electron), the Electron + Bare worker template [Keet](https://keet.io) and [PearPass](https://pass.pears.com) ship. **One-time** multisig wiring is in [Set up multisig](/pear/how-to/operate-an-app/multisig/set-up-multisig); every production shipment after that uses [Sign with multisig](/pear/how-to/operate-an-app/multisig/sign-with-multisig). If a commit fails mid-flight, see [Troubleshoot multisig](/pear/how-to/operate-an-app/multisig/troubleshoot-multisig).

### How setup connects to the release cycle

The [multisig config](/pear/how-to/operate-an-app/multisig/set-up-multisig#create-the-multisig-config) in `pear.json` derives the **multisig link** from its `namespace`, `publicKeys`, and `quorum` alone—it does **not** reference the provision drive. The provision link from the most recent [`hello-pear-electron`](https://github.com/holepunchto/hello-pear-electron) `pear stage` / [`pear provision`](/pear/how-to/operate-an-app/manual-deployment/deployment#6-provision) is instead supplied as the **source** when you prepare and commit each signing request. Shipped binaries read the **multisig link** from [`package.json` `upgrade`](/pear/how-to/operate-an-app/multisig/set-up-multisig#set-upgrade-to-the-multisig-link).
...
Each production commit runs the four steps in [Sign with multisig](/pear/how-to/operate-an-app/multisig/sign-with-multisig): [`pear multisig request`](/pear/how-to/operate-an-app/multisig/sign-with-multisig#prepare-the-request), [`pear multisig sign`](/pear/how-to/operate-an-app/multisig/sign-with-multisig#sign) from each signer until [quorum](/pear/how-to/operate-an-app/multisig/set-up-multisig#create-the-multisig-config) is met, [`pear multisig verify`](/pear/how-to/operate-an-app/multisig/sign-with-multisig#verify), then [`pear multisig commit`](/pear/how-to/operate-an-app/multisig/sign-with-multisig#commit).
...
For [`hello-pear-electron`](https://github.com/holepunchto/hello-pear-electron), one [`pear build`](/pear/how-to/operate-an-app/manual-deployment/deployment#4-build-the-deployment-directory) output can feed parallel **stage** links; **prerelease** and **production** are the provision and multisig gates on the trust ladder.
...
Platform installs still use Pear’s OS-wide tree described in [Storage and distribution](/pear/explanation/storage-and-distribution). The diagrams here are about **application release artifacts** moving between links—not replacing that global layout.
...
| **Application drive** | The [Hyperdrive](/p2p/reference/building-blocks/hyperdrive) that holds the published app files and version history for a given `pear://` link. |
...
| **Pear link** | Stable `pear://…` identifier the swarm routes on; see the **Links** row in [Command Line Interface](/pear/reference/pear/cli) and [Storage and distribution](/pear/explanation/storage-and-distribution). |
...
| **Versioned link** | A `pear://` link that pins `fork`, `length`, and `key` of the [Hypercore](/p2p/reference/building-blocks/hypercore) behind the application Hyperdrive—used when provisioning between known versions. |

## Where to go next

- [Pear desktop application architecture](/pear/explanation/pear-desktop-architecture)—workers, storage, and update events in the running app.
- [Deploy your application](/pear/how-to/operate-an-app/manual-deployment/deployment)—operator checklist for `pear stage` / `pear provision` / multisig tooling.
- [Build desktop distributables](/pear/how-to/operate-an-app/build-and-package/build-desktop-distributables)—per-OS makes and signing pointers.
- [Desktop release npm scripts](/pear/reference/ci-and-release/desktop-release-npm-scripts)—common `package.json` scripts.
- [Troubleshoot desktop releases](/pear/how-to/operate-an-app/manual-deployment/troubleshoot-desktop-releases)—OTA, staging, and `pear build` pitfalls.
