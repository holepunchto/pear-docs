...
A running Pear desktop app polls the [Hyperdrive](/bare/reference/building-blocks/hyperdrive) behind its `upgrade` link, downloads new application data when the [Hypercore](/bare/reference/building-blocks/hypercore) length advances, and emits two events from [`pear.updater`](/pear/reference/pear/runtime). The host process forwards them to renderers so the UI can react:
...
| **Application drive** | The [Hyperdrive](/bare/reference/building-blocks/hyperdrive) that holds the published app files and version history for a given `pear://` link. |
...
| **Versioned link** | A `pear://` link that pins `fork`, `length`, and `key` of the [Hypercore](/bare/reference/building-blocks/hypercore) behind the application Hyperdrive—used when provisioning between known versions. |
