...
`pear stage` writes new content into the [Hypercore](/p2p/reference/building-blocks/hypercore) but does **not** move the `release` pointer that peers poll. Staging ends by printing the new **`Latest`** length and the post-stage versioned link:
...
The reference wiring lives in [Read the Electron main process](/p2p/getting-started/build-a-peer-to-peer-chat/reshape-into-a-production-app#read-the-electron-main-process) (the per-worker `before-quit` listeners and the `app:afterUpdate` handler that calls `app.relaunch()` then `app.quit()`).
