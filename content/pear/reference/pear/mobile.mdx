...
- `dir` `<String>` - Directory to store runtime data. Defaults to the platform's persistent app-data directory (resolved via [`bare-storage`](/bare/reference/bare/modules)'s `persistent()`—iOS: the app's Application Support directory; Android: the app's `files` directory).
