...
Pear is built on [Bare](/bare/reference/modules/bare-modules). Pear applications have a `global.Pear` object and a`global.Bare` object. For background on the runtime split, see [Runtime and languages](/pear/explanation/runtime-and-languages); for the surrounding `pear-*` library catalog, see [Modules](/pear/reference/modules/pear-modules).
...
**REMOVED**—use [`Bare.argv`](#bare-argv).
...
**REMOVED**—use [`Bare.exitCode`](#bare-exitcode).
...
**REMOVED**—use [`Bare.exit()`](#bare-exit).

Exits the process with the provided exit code. Follows Pear teardown flow, whereas [`Bare.exit()`](#bare-exit) does not.
...
The majority of capabilities are supplied via [Bare modules](/bare/reference/modules/bare-modules).

### `Bare.platform`
<a name="bare-platform"></a>
...
### `Bare.exitCode`
<a name="bare-exitcode"></a>

The code that will be returned once the process exits. If the process is exited using `Bare.exit()` without specifying a code, `Bare.exitCode` is used.

### `Bare.version`
<a name="bare-version"></a>

The Bare version string.

### `Bare.versions`
<a name="bare-versions"></a>
...
<a name="bare-on-uncaught-exception"></a>
...
<a name="bare-on-unhandled-rejection"></a>
...
### `addon.url`
<a name="bare-addon-url"></a>

The WHATWG `URL` identifier of the addon.

### `addon.exports`
<a name="bare-addon-exports"></a>

The exports of the addon.

## `Bare.Thread`
<a name="bare-thread"></a>
...
Optional streaming communication between an embedder and JavaScript code. By default, `Bare.IPC` is `null`, indicating streaming communication isn't supported. If set by the embedder, `Bare.IPC` is a [`bare-stream`](https://github.com/holepunchto/bare-stream) `Duplex`.

This is how [Pear OTA](/pear/reference/pear/runtime) wires up worker communication: [`PearRuntime.run()`](/pear/reference/pear/runtime#running-workers) returns the embedder-side `IPC` duplex stream, and the spawned worker reads and writes the other end as `Bare.IPC`.
