...
Pear is built on [Bare](/bare/reference/bare/modules). Pear applications have a `global.Pear` object. For background on the runtime split, see [Runtime and languages](/pear/explanation/runtime-and-languages); for the surrounding `pear-*` library catalog, see [Modules](/pear/reference/modules/pear-modules). For the `global.Bare` object, see the [Bare runtime API](/bare/reference/bare/runtime).
...
- [Bare modules](/bare/reference/bare/modules)—runtime primitives the API builds on.
