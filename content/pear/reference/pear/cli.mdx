...
<Callout type="info">
<Since v="3.5.0" label="Fixed in 3.5.0" /> The table is written straight to stdout instead of through `console.log`, which mangled its non-ASCII characters on Windows—the header and footer rules and the `✔` marks came out as mojibake. Other platforms were unaffected, and the table's content and layout are unchanged.
</Callout>
