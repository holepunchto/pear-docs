...
<Callout type="info">
This page tracks **Pear 3.5.0**. Several commands were removed in v3—most notably `pear run`, which is replaced by embedding the **[Pear OTA](/pear/reference/pear/runtime)** (`pear-runtime`) library. Removed commands are kept below with a badge and migration guidance.

Anything a release adds or removes carries an inline badge—<Since v="3.1.0" /> or <Until v="3.1.0" />—so you can tell whether your build has it. Run `pear versions` to check what you are on, and see the [Release Overview](/pear/release-overview) for the full history.
</Callout>
...
This bootstraps Pear onto the system. The `curl`/`irm` installers fetch a `checksums.sha256` manifest alongside the platform binary and verify its SHA-256 before moving it into place, so a corrupted or tampered download is rejected rather than installed. Complete the setup by following the `PATH` instructions the installer prints, then run `pear` directly. To install it globally through npm instead, use `npm i -g pear`—and every `pear <cmd>` also works as `npx pear <cmd>`, which is handy for running commands from a directory other than the one you installed into. npm and Node.js are only the delivery mechanism: after install, `pear` runs on [Bare](/bare/reference/bare/runtime), not Node.
...
Run `npx pear` to move onto the v3 platform. From v3 on, the CLI is a standalone binary that keeps itself up to date over the air through [Pear OTA](/pear/reference/pear/runtime)—so you no longer reinstall from npm for platform updates. Several commands were removed in v3 (see the <Status level="removed" /> badges below); if your application relied on the removed `pear run` runtime or the global `Pear` API, follow [Migrate from pear run to Pear OTA](/pear/how-to/operate-an-app/migration) and review the [Release Overview](/pear/release-overview).
...
<Since v="3.5.0" /> All four [`pear identity`](#pear-identity) subcommands print a hint too, naming the command their key is usually passed to. On 3.4.0 the only non-deployment command with a hint was `pear blind-peer identity`, which 3.5.0 removed.
...
| [`pear blind-peer`](#pear-blind-peer) <Since v="3.4.0" /> | **before** the subcommand | `pear blind-peer --json request <key> --peer <peer>` | `pear blind-peer request <key> --peer <peer> --json` |
| [`pear identity`](#pear-identity) <Since v="3.5.0" /> | **before** the subcommand | `pear identity --json seed` | `pear identity seed --json` |
...
For the full release walkthrough, see [Deploy a Pear desktop app](/pear/how-to/operate-an-app/manual-deployment/deployment) and [Build desktop distributables](/pear/how-to/operate-an-app/build-and-package/build-desktop-distributables).
...
`--ignore` supports glob patterns (`*`, `**`) and a leading `!` to un-ignore. `--only` does not—it matches by exact path or directory prefix. Both **add to** rather than replace the corresponding list in [`pear.json`](/pear/reference/pear/configuration), and `--purge` also switches on automatically when `pear.json` sets `stage.purge`.
...
Production signing coordination. Gate production releases behind a quorum of signing keys instead of a single machine's key. Since Pear v3 removed the single-key `pear release` command, quorum multisig (together with [`pear provision`](#pear-provision)) is the path to production. See [Set up multisig](/pear/how-to/operate-an-app/multisig/set-up-multisig) and [Sign with multisig](/pear/how-to/operate-an-app/multisig/sign-with-multisig) for the end-to-end flow.
...
`--config` points at a file that must carry a `multisig` field with `publicKeys`, `quorum`, and `namespace`—see [Pear Application Configuration](/pear/reference/pear/configuration).
...
Seed a link so peers can fetch it, then install the app it points to. `pear run` launched an app directly from a link in v2; apps now embed [Pear OTA](/pear/reference/pear/runtime) instead.
...
<Since v="3.2.0" /> `--until-sync <key>` takes a peer's public key (z32) and exits automatically once every specified peer has fully synced—pass the flag multiple times to wait for more than one peer. Useful for scripting a seed session that should end on its own instead of running indefinitely. The key to pass is the one printed after each peer join or peer sync line in the log below the stats table—or, for the machine you are on, what [`pear identity seed`](#pear-identity) prints <Since v="3.5.0" />.

The live view is a stats table above a log of peer and announce lines:

<VersionGate since="3.5.0">
```
Seeding:       pear://q9bhn175f34ahxnriujd68ykiww3syzmdf9w4humxoxqeshjhy6o
Verlink:       pear://0.3.q9bhn175f34ahxnriujd68ykiww3syzmdf9w4humxoxqeshjhy6o
App:           seedcheck@1.2.3
Drive Length:  3 [100%]
Blobs Length:  4 [100%]
Blobs Size:    131B
Discovery Key: m5xisy4tfmd7wgnoxpjygdnynbweti7pb5d5tc8zb3zxaafzoz3y
Content Key:   7qbpsi5hygtbppejymfnzitiygpysndgbrbm54og11d1gcqsdupo
Firewalled:    true
NAT Type:      consistent
Whoami:        4ibhiwr1pstsw8h4gt3gti88m3de4zqb76bzsjjyomc9woipdbto
Network:       [ Peers 0 ]  [ ⬆ 0B - 0B/s ]  [ ⬇ 0B - 0B/s ]
──────────────────────────────────────────────────────
^_^ announced
```

The gap between labels and values is computed from the terminal width, so it will not match this sample exactly. Under `--no-tty` the same fields print as one `... <field> <value>` line each, re-emitted on every stats tick instead of redrawn in place.
</VersionGate>

<Since v="3.4.0" /> `--blind-peer <key>` takes a [blind peer](/p2p/explanation/blind-peering)'s public key (z32)—read off the blind peer's machine with [`pear identity blind-peer`](#pear-identity) <Since v="3.5.0" />—and registers the drive being seeded with it, so it stays available after this session ends. The live log shows `adding core`, `confirming add core`, and `added core` lines as the request progresses. Registering does not end the session: the blind peer's key is added to the `--until-sync` set, so `pear seed` then waits for that peer to fully sync the drive's cores before exiting—on a large drive that is a long wait, not a quick handshake. Reaching the blind peer at all has its own 30-second budget, after which the command fails with `ERR_OPERATION_FAILED`, `Timed out connecting to blind peer <key>`. Passing both flags is fine; the blind peer is simply one more key to wait for. This is a one-flag alternative to wiring the [`blind-peering`](https://github.com/holepunchto/blind-peering) client library directly for the common case of registering one drive—see [Keep data available with blind peering](/p2p/how-to/blind-peering/keep-data-available-with-blind-peering) for when you'd still reach for the library instead (multiple cores, Autobases, or finer control).

<VersionGate since="3.4.0">
<Callout type="warn">
The request asks the blind peer to **announce** the drive, but that only sticks if the blind peer trusts you—its operator has to have started it with `--trusted-peer <your-key>`, where your key is what [`pear identity blind-peer-client`](#pear-identity) prints on this machine (`pear blind-peer identity` <Until v="3.5.0" /> before that). Against a blind peer that does not trust you the drive is still stored and served, but never announced, so peers will not discover it through that blind peer. Nothing in the command's output distinguishes the two cases; check the blind peer's own logs for a downgraded announce.
</Callout>
</VersionGate>
...
<Callout type="info">
<Since v="3.5.0" /> The terminal stats table replaces the `Drive Key:` row with `Verlink:`, which prints the full versioned link (`pear://<fork>.<length>.<key>`) rather than the bare key. The key is still there—it is the last segment—and [`--json`](#json-output) still reports `driveKey` unchanged, so scripts reading that field are unaffected. There is no `verlink` field in `stats`: the terminal row is composed from `driveFork`, `driveLength`, and `driveKey`, so a script that wants it builds it the same way. On 3.4.0 and earlier the table carries `Drive Key:` and no verlink.
</Callout>

<Callout type="info">
<Since v="3.5.0" /> `Drive Length:` reports sync progress rather than a bare block count, and a `Blobs Length:` row is added alongside it. Each shows `<synced>/<total>` and a percentage while syncing, collapsing to just `<total>` once complete—so `12/40 [ 30%]` means 12 of 40 blocks are local, and `3 [100%]` means all 3 are. In-progress rows are highlighted, completed ones dimmed. The percentage is right-aligned to three digits in the live view (`[100%]`, `[ 30%]`) and spaced as `[ 30% ]` under `--no-tty`. The `--json` `stats` data gains `driveSynced`, `blobsSynced`, `blobsLength` and `driveFork` alongside the existing `driveLength`. On 3.4.0 and earlier `Drive Length:` is a plain number and there is no blobs block count.
</Callout>
...
- **Applications** go to `~/Applications` on macOS. On Linux Pear uses `~/Applications` or `~/AppImages` if one exists, and falls back to `~/.local/bin`. On **Windows**, an application installs as a registered MSIX package instead of being copied anywhere—there is no destination directory, so `--to` has no effect on it.
...
- `--to <dir>` overrides all of the above, except a Windows application install, which always goes through MSIX regardless.
...
See [Storage and distribution](/pear/explanation/storage-and-distribution#installing-applications) for the install model and the direction described in [Pear Evolution](https://pears.com/news/pear-evolution/).
</Callout>
...
Applications are now launched by embedding the **[Pear OTA](/pear/reference/pear/runtime)** (`pear-runtime`) library, which provides over-the-air peer-to-peer updates, application storage, and Bare workers. See [Migrate from `pear run` to Pear OTA](/pear/how-to/operate-an-app/migration) for the move.

## Blind peers and relays
<VersionSection since="3.4.0" />

<Since v="3.4.0" /> Two commands bring peer-to-peer availability and connectivity services natively into the platform CLI: **blind peers**, which replicate cores on your behalf without reading them (see [Blind peering](/p2p/explanation/blind-peering)), and **blind relays**, which forward connections between peers that cannot hole-punch directly (see [Peer-to-peer, demystified](/p2p/explanation/peer-to-peer-demystified#hole-punching-and-nat-traversal)). Both existed as separate packages before 3.4.0; these commands run the same functionality through `pear` directly.
...
| `identity` <Until v="3.5.0" /> | Show peer identity key—replaced by [`pear identity blind-peer-client`](#pear-identity) |
...
`--json` is declared on `pear blind-peer` itself, so it goes **before** the subcommand—`pear blind-peer --json start`, not `pear blind-peer start --json` (see [`--json` position](#json-output)). Note that [`pear blind-relay`](#pear-blind-relay) takes it the other way round.
...
| A client's identity key | [`pear identity blind-peer-client`](#pear-identity) <Since v="3.5.0" /> (`pear blind-peer identity` <Until v="3.5.0" /> before that), run **on the client machine** | The **authorization**: what the blind peer's operator passes to `start --trusted-peer <peer>` |
...
<Until v="3.5.0" label="Removed in 3.5.0" /> `pear blind-peer identity` printed **this machine's** sidecar public key—the identity a blind peer operator authorizes with `--trusted-peer`. From 3.5.0 it is [`pear identity blind-peer-client`](#pear-identity), which prints the same key; the old form fails with `✖ Unrecognized Argument at index 0 with value identity`.
...
This is the platform-native equivalent of running the standalone [`blind-peer-cli`](https://github.com/holepunchto/blind-peer-cli) package—see [Blind peering](/p2p/explanation/blind-peering#running-a-blind-peer-server) for the operational picture. Note that the disk budget described there is a `blind-peer-cli` flag (`-m <size>`); `pear blind-peer start` exposes no equivalent and uses the library default.
...
Like [`pear blind-peer`](#pear-blind-peer), only one relay runs per sidecar—starting a second fails with `ERR_OPERATION_FAILED`, `Blind relay is already running, cannot start another`. The relay's key pair is derived from the platform corestore, so a given machine keeps the same relay key across restarts—[`pear identity blind-relay`](#pear-identity) <Since v="3.5.0" /> prints it without having to scrape `start`'s output.
...
Any command accepts `--relay <key>`, where `<key>` is a running [`pear blind-relay`](#pear-blind-relay)'s public key—read off the relay machine with [`pear identity blind-relay`](#pear-identity) <Since v="3.5.0" />. An invalid key fails fast, before the command itself runs: `--relay <key> must supply a valid public key`.
...
Reach for this when direct hole-punching isn't working—see [Relay connections through a blind relay](/p2p/how-to/connect-to-peers/relay-connections-through-a-blind-relay) for the end-to-end setup, and [Hole punching and NAT traversal](/p2p/explanation/peer-to-peer-demystified#hole-punching-and-nat-traversal) for why it fails.
...
Supply a link to view an application's changelog, read from the `CHANGELOG.md` at its drive root. Any Pear app can ship one—see [Publish a changelog for your app](/pear/how-to/operate-an-app/publish-a-changelog) for the file format and how to stage it.
...
───────────────────────────────────────────────────────────────────────────────────────────────────────
 seedcheck  core   pear://q9bhn175f34ahxnriujd68ykiww3syzmdf9w4humxoxqeshjhy6o  3       ✔         347B
 seedcheck  blobs  pear://7qbpsi5hygtbppejymfnzitiygpysndgbrbm54og11d1gcqsdupo  4       ✔         131B
───────────────────────────────────────────────────────────────────────────────────────────────────────
Total cores: 2 (writable: 2)
Total size:  478B
```

Each column is padded to its widest value, so the rules and the column positions shift with the content. A separator rule also appears between cores belonging to different drives when more than one application is listed. `blobs` rows render dimmed. The empty-store message is now `[ No cores ]`, replacing the bare `No cores` used through 3.3.0.
</VersionGate>

<Callout type="info">
<Since v="3.5.0" label="Fixed in 3.5.0" /> The table is written straight to stdout instead of through `console.log`, which mangled its non-ASCII characters on Windows—the header and footer rules and the `✔` marks came out as mojibake. macOS output was already correct, and the table's content and layout are unchanged.
</Callout>
...
Manage the background sidecar process, read this machine's network keys, reclaim disk space, and check what's installed.
...
### `pear identity [command]`
<VersionSection since="3.5.0" />

<a name="pear-identity"></a>

<Since v="3.5.0" /> Show local network public keys. For the workflows these keys feed into, see [Look up this machine's network keys](/pear/how-to/look-up-network-keys).
...
Each subcommand prints one z32 key and nothing else. Run without a subcommand, `pear identity` prints its own help rather than defaulting to a key.

| Subcommand | The key | Pass it to |
| --- | --- | --- |
| `seed` | This machine's sidecar key pair—the same value [`pear seed`](#pear-seed) reports as `Whoami:` | [`pear seed --until-sync <key>`](#pear-seed), run by whoever is waiting on this machine to sync |
| `blind-relay` | This machine's relay key pair, derived from the platform corestore and stable across restarts | [`--relay <key>`](#relay-flag) on another machine, once [`pear blind-relay start`](#pear-blind-relay) is running here |
| `blind-peer` | The blind peer server's own key on this machine—the same value [`pear blind-peer start`](#pear-blind-peer) prints as `Blind peer started listening using public key:` | [`pear seed --blind-peer <key>`](#pear-seed) or `pear blind-peer request --peer <key>` |
| `blind-peer-client` | This machine's DHT key pair, the identity it presents *to* a blind peer | `pear blind-peer start --trusted-peer <peer>`, run by the blind peer's operator |

The four keys are distinct and not interchangeable—see the [two-keys table](#pear-blind-peer) for the pairing that trips people up most, `blind-peer` (the target) against `blind-peer-client` (the authorization).

The key is written to **stdout** and the [next-step hint](#next-step-hints) to **stderr**, so `KEY=$(pear identity seed)` captures the key alone. Each subcommand prints a different hint: `seed` suggests `pear seed --until-sync=<key> <link>`, `blind-relay` suggests `pear --relay <key> seed <link>`, `blind-peer` suggests `pear seed --blind-peer=<key> <link>`, and `blind-peer-client` suggests `pear blind-peer start --trusted-peer=<key>`.

`--json` is declared on `pear identity` itself, so it goes **before** the subcommand—`pear identity --json seed`, not `pear identity seed --json` (see [`--json` position](#json-output)). Output is a single `final` object:
...
<Callout type="info">
`seed`, `blind-relay` and `blind-peer-client` read key pairs the sidecar already holds, so they are pure lookups. `blind-peer` is not: if no blind peer is running, it opens one against the platform `blind-peer` storage directory to read its key, creating that directory on first use. The key is stable, so repeated calls return the same value.
</Callout>
...
`pear drop` was tied to the removed `pear run` model. See [Manage installed applications](/pear/how-to/manage-installed-applications) for resetting application storage.
...
<Callout type="info">
To scaffold a project, clone the official template instead—[`holepunchto/hello-pear-electron`](https://github.com/holepunchto/hello-pear-electron)—and follow [Start from the hello-pear-electron template](/p2p/getting-started/from-a-template/start-from-hello-pear-electron).
</Callout>

## See also

- [Release Overview](/pear/release-overview)—the curated record of the v3 command removals and other module changes.
- [Manage installed applications](/pear/how-to/manage-installed-applications)—resetting and inspecting installed apps.
- [Distribute as a binary](/pear/how-to/operate-an-app/build-and-package/distribute-as-binary)—packaging a release built from these commands.
- [Pear OTA reference](/pear/reference/pear/runtime)—the `pear-runtime` library that replaces `pear run`.
- [Blind peering](/p2p/explanation/blind-peering)—the concept behind `pear blind-peer` and `pear seed --blind-peer`.
- [Peer-to-peer, demystified](/p2p/explanation/peer-to-peer-demystified)—hole punching and the relay paths `pear blind-relay` and `--relay` provide as a fallback.
