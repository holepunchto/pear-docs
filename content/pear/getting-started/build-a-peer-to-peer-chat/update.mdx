...
4. The header now reads **"Pear chat—v2"** and the version label shows `v1.0.2`. The [Corestore](/bare/reference/helpers/corestore)-backed chat transcript replays from disk.
...
A multisig drive is a [Hypercore](/bare/reference/building-blocks/hypercore) where write access is gated by a quorum of signing keys instead of a single owner machine. This is what production Pear apps use so no single laptop can push a malicious update.
