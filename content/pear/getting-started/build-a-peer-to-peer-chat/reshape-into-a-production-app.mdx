...
<Callout type="info">
The worker's internals—`WorkerTask`'s Corestore/Hyperswarm wiring, the Autobase-backed `ChatRoom` with blind-pairing invites, and the generated `spec/` schema—are pure peer-to-peer logic with nothing Pear-specific about it. [A production Autobase-backed chat room](/bare/explanation/autobase-backed-chat-room) traces all three in detail. The rest of this tutorial covers the Electron shell around that worker.
</Callout>
...
| Room | `workers/chat-room.js` | [A production Autobase-backed chat room](/bare/explanation/autobase-backed-chat-room) |
...
- [Add blind peering to a chat app](/bare/how-to/blind-peering/add-blind-peering-to-a-chat-app)—keep the room online when its writers are offline.
- [Add Keet identity to a chat app](/bare/how-to/manage-identity/add-keet-identity-to-a-chat-app)—anchor messages to a portable identity key.
- [Host multiple rooms in one chat app](/bare/how-to/connect-to-peers/host-multiple-rooms-in-one-chat-app)—extend from one room to an account with many.
- [Share files in a peer-to-peer app](/bare/how-to/stream-and-share-media/share-files-in-a-peer-to-peer-app)—swap the room's HyperDB view for a [Hyperdrive](/bare/reference/building-blocks/hyperdrive).

Or zoom out to the conceptual picture:

- [A production Autobase-backed chat room](/bare/explanation/autobase-backed-chat-room)—the worker-side Corestore/Hyperswarm/Autobase/blind-pairing internals this tutorial builds around.
