...
This is **part 3 of 4** in the [getting started path](/pear/getting-started). You take the production-shaped app from [part 2](/pear/getting-started/build-a-peer-to-peer-chat/reshape-into-a-production-app) and walk it through the first half of Pear's release pipeline:
...
<Callout type="info">
[Deploy over-the-air updates](/pear/getting-started/build-a-peer-to-peer-chat/update) (part 4 of 4) continues from here and demonstrates the live OTA cycle on the provision link, plus a tour of multisig.
</Callout>
...
You need:
1. The working production-shaped app from [part 2](/pear/getting-started/build-a-peer-to-peer-chat/reshape-into-a-production-app).
...
[`pear touch`](/pear/reference/pear/cli) mints a **new `pear://` link you own**—one backed by a fresh [Hypercore](/bare/reference/building-blocks/hypercore) whose write key is stored locally on this machine. This becomes your **stage link**: the append-only core you sync builds into before each provision. (The link you set in part 2 was a public placeholder so the app would boot; you replace it now with your own.)
...
Run these from the **parent directory** of your app (same place you ran `pear build`) unless noted. [`pear stage`](/pear/reference/pear/cli) syncs the deployment directory into the [Hypercore](/bare/reference/building-blocks/hypercore) behind your **stage link**.
...
| `pear stage` | Append-only sync into the stage [Hypercore](/bare/reference/building-blocks/hypercore) | History is permanent; updates are not |
...
- **Continue the path:** [Deploy over-the-air updates](/pear/getting-started/build-a-peer-to-peer-chat/update) (part 4 of 4)—run the installed build, ship a second version, watch OTA fire end-to-end, and preview multisig.
