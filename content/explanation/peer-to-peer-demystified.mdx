...
[HyperDHT](#hyperdht-discovery-and-encrypted-connections) implements this discovery and connection machinery. You rarely call hole punching directly; you create a DHT node or join a Hyperswarm topic and the stack handles traversal.
...
[Hyperswarm](/reference/building-blocks/hyperswarm) wraps HyperDHT with a higher-level API oriented around **topics**—arbitrary 32-byte identifiers, often a Hypercore's `discoveryKey`. Where HyperDHT exposes servers, clients, announce streams, and lookup streams, Hyperswarm exposes `join`, `leave`, and a single `connection` event. 
