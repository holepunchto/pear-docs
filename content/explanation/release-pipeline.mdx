...
The two-event split is what lets you build UI like the "Update ready!" banner in [Reshape into a production app](/getting-started/build-a-peer-to-peer-chat/production-shape): show a spinner on `updating`, swap to a restart button on `updated`, restart the app on click.
...
[`multisig.json`](/how-to/operate-an-app/multisig/set-up-multisig#create-the-multisig-config) points at the **provision drive** (`srcKey`) from the most recent [`hello-pear-electron`](https://github.com/holepunchto/hello-pear-electron) `pear stage` / [`pear provision`](/how-to/operate-an-app/manual-deployment/deployment#6-provision). Shipped binaries read the **multisig link** from [`package.json` `upgrade`](/how-to/operate-an-app/multisig/set-up-multisig#set-upgrade-to-the-multisig-link).

```mermaid
graph TB
  subgraph Setup[Multisig setup]
    K(7a. Create signing keys) --> C(7b. Create multisig.json)
    C --> L(7c. Set upgrade to multisig link)
  end
  ...
  Prov -->|assessed| MS(7d–7g. Multisig)
  MS --> Live[Production live]
```

### Quorum signing

Each production commit runs the four steps in [Sign with multisig](/how-to/operate-an-app/multisig/sign-with-multisig): [`hyper-multisig request`](/how-to/operate-an-app/multisig/sign-with-multisig#prepare-the-request), [`hypercore-sign`](/how-to/operate-an-app/multisig/sign-with-multisig#sign) from each signer until [quorum](/how-to/operate-an-app/multisig/set-up-multisig#create-the-multisig-config) is met, [`hyper-multisig verify`](/how-to/operate-an-app/multisig/sign-with-multisig#verify), then [`hyper-multisig commit`](/how-to/operate-an-app/multisig/sign-with-multisig#commit).
