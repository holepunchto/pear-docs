...
flowchart BT
  ...
  hyperblobs["Hyperblobs — large opaque objects"]
  hyperdrive["Hyperdrive — path-addressed files"]

  hypercore --> hyperblobs
  hyperblobs --> hyperdrive
```
...
[Hyperblobs](https://github.com/holepunchto/hyperblobs) chunks large data across Hypercore blocks and exposes a simple put/get API. A `put` returns an **id** (start, end, length, and related metadata) that you store elsewhere — for example in a [Hyperbee](/reference/building-blocks/hyperbee) key.
