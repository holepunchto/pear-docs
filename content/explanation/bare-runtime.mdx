...
Bare is built on two C libraries: [`libjs`](https://github.com/holepunchto/libjs), which provides low-level bindings to V8 in an engine-independent way, and [`libuv`](https://github.com/libuv/libuv), which provides the asynchronous I/O event loop. On top of those primitives Bare adds only a few missing pieces:
...
No. A Bare program can be compiled into a single standalone executable with `bare-build`—this is how the [`hello-pear-bare`](/getting-started/from-a-template/start-from-hello-pear-bare) template ships—so no Node.js, Bare, or Pear CLI is required on the user's machine.

### Which JavaScript engine does Bare use?

It depends on the build. Bare talks to the engine through the `libjs` ABI, so it can be compiled against V8 (the default) or JavaScriptCore via `libjsc`. Your application and addons don't change between engines.
