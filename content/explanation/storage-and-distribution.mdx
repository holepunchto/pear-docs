...
That runtime capability is being unbundled from the CLI: `pear run` is deprecated in favour of the embeddable [`pear-runtime`](/reference/pear/runtime) library, so any JavaScript project can carry peer-to-peer over-the-air updates without depending on the CLI. The CLI's remaining focus is deployment and, now, installation. The full rationale is in [Pear Evolution](https://pears.com/news/pear-evolution/).
