...
The directories above are the **platform**'s storage. An **application** embedded in [Electron](/getting-started/build-a-peer-to-peer-chat/production-shape) — the `dir` field passed to `new PearRuntime({ dir, ... })` — lives in a different per-OS location, and it differs between development and packaged builds:
...
The convention comes from the getting-started [production-shape `getPear()` helper](/getting-started/build-a-peer-to-peer-chat/production-shape): when `app.isPackaged` is `false`, the dir falls back to `os.tmpdir()`; otherwise it picks the OS-appropriate user-data folder. (In the [`hello-pear-electron`](https://github.com/holepunchto/hello-pear-electron/blob/main/electron/main.js) template the same logic lives in `getWorker()`, which passes `dir` to the Bare worker that constructs the runtime.) The Pear runtime exposes that dir as `pear.storage`, which the worker passes to [Corestore](/reference/helpers/corestore) as `storage`.
