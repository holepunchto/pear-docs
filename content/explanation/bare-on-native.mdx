...
Yes. `bare-kit` embeds a Bare worklet in native iOS and Android apps today, and `react-native-bare-kit` does the same for React Native and Expo. What's still landing is the built-in `pear-runtime` equivalent for mobile (noted earlier on this page).
