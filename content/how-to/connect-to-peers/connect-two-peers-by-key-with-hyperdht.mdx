...
[`HyperDHT`](/reference/building-blocks/hyperdht) helps clients connect to a server peer with a known public key. HyperDHT uses a series of holepunching techniques to establish direct connections between the peers, even if they're located on home networks with tricky NATs.
...
Use the HyperDHT to create a basic CLI chat app where a client peer connects to a server peer by public key. The example consists of two applications: [`client-app`](#create-the-client-app) and [`server-app`](#create-the-server-app).
...
Run the `server-app` with `bare server-app`.
