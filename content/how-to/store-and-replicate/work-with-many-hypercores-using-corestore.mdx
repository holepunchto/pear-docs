...
<Callout type="info">
Only one Corestore per application is needed. This is a recommended best practice for efficient Hypercore management and to avoid pitfalls from multiple Corestores. A single Corestore:

- Manages multiple sessions for the same Hypercore.
...
```javascript example=corestore-multi step=write file=multicore-writer-app/index.js
...
```javascript example=corestore-multi step=write file=multicore-reader-app/index.js
import process from 'bare-process'
import Corestore from 'corestore'
import Hyperswarm from 'hyperswarm'
import b4a from 'b4a'
...
- [Apply recommended practices](/how-to/operate-an-app/recommended-practices) — the Corestore-per-app guidance in context.
