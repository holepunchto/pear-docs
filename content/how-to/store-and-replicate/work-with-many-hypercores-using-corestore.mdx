...
<Callout type="info">
This guide is a terminal-only walkthrough. For the **desktop equivalent**—the same Corestore co-replication pattern wired into an Electron + Bare worker shell—follow the [Getting Started path](/getting-started), which builds a `pear://` chat app on top of the [`hello-pear-electron` template](/getting-started/from-a-template/start-from-hello-pear-electron).
</Callout>
