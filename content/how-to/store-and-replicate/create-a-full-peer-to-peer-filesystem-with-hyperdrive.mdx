...
Now let's mirror a local directory into a Hyperdrive, replicate it with a reader peer, who then mirrors it into their own local copy. When the writer modifies its drive, by adding, removing, or changing files, the reader's local copy will be updated to reflect that. To do this, we'll use two additional tools: [`MirrorDrive`](/reference/helpers/mirrordrive) and [`LocalDrive`](/reference/helpers/localdrive), which handle all interactions between Hyperdrives and the local filesystem.
