...
This is because Hyperbee only downloads the [Hypercore](/reference/building-blocks/hypercore) blocks it needs to satisfy each query, a feature called **sparse downloading**.
</Callout>
...
- [Build a multiwriter app with Autobase](/how-to/store-and-replicate/build-a-multiwriter-app-with-autobase)—let several peers write to one shared view instead of a single writer.
- [Build a multiwriter database with Autobee](/how-to/store-and-replicate/build-a-multiwriter-database-with-autobee)—give this same B-tree many writers.
