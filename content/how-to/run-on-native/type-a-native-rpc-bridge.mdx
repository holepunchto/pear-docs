...
<Callout type="info">
This guide shows the `bare-rpc` + `hyperschema` primitives directly, which is what you need when the far side is written in a language [HRPC](/reference/helpers/hrpc) has no generator for yet—Kotlin today. Before following it, check whether HRPC already covers your case, because it usually does:
...
See [Structured RPC over IPC](/explanation/workers#structured-rpc-over-ipc).
</Callout>
