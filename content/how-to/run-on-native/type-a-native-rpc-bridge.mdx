...
<Callout type="info">
This guide builds the seam directly on `bare-rpc` because the native side needs the schema compiled to another language, and `bare-rpc` is where the Kotlin and C generators are heading. If **both endpoints are JavaScript**—a Pear app's host and its worker, or a React Native view and its Bare worklet—use [HRPC](/reference/helpers/hrpc) instead: it generates the same kind of typed stubs shown below without hand-writing the dispatch. Note this is about the endpoints, not the shell: an RN app whose two ends are JS still counts as JavaScript-to-JavaScript. For a **Swift** shell, [`hrpc-swift`](https://github.com/holepunchto/hrpc-swift) generates a wire-compatible Swift `HRPC` class from the same schema, so HRPC covers that case too. See [Structured RPC over IPC](/explanation/workers#structured-rpc-over-ipc).
</Callout>
