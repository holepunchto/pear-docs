...
<Callout type="info">
This guide builds the seam directly on `bare-rpc` because it also needs to generate Swift/Kotlin codecs for the native side. If both ends are JavaScript—no native shell involved, just a Pear app's host and worker—use [HRPC](/reference/helpers/hrpc) instead: it generates the same kind of typed stubs shown below without hand-writing the dispatch. See [Structured RPC over IPC](/explanation/workers#structured-rpc-over-ipc).
</Callout>
