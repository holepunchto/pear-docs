...
If `Latest` is higher than `Release`, peers continue to fetch the older content. Run `pear release pear://<your-link>` to advance the pointer. For production releases, use [`pear provision`](/how-to/operate-an-app/deployment#6-provision) or `pear multisig` instead — both manage their own release pointers.
...
The reference wiring lives in [Replace `electron/main.js` with the instance form](/getting-started/production-shape#replace-electronmainjs-with-the-instance-form) (the `destroyWorkers` helper and the `before-quit` listener).
...
Then update `srcKey` in `multisig.json` to the new provision target's key (see [step 7b. Create the multisig config](/how-to/operate-an-app/deployment#7b-create-the-multisig-config)).
