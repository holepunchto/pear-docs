...
The reference wiring lives in [Replace `electron/main.js` with the instance form](/getting-started/build-a-peer-to-peer-chat/production-shape#read-the-electron-main-process) (the `destroyWorkers` helper and the `before-quit` listener).
...
Then update `srcKey` in `multisig.json` to the new provision target's key (see [Create the multisig config](/how-to/operate-an-app/multisig/set-up-multisig#create-the-multisig-config)). For multisig-specific recovery — interrupted commits and `INCOMPATIBLE_SOURCE_AND_TARGET` — see [Troubleshoot multisig](/how-to/operate-an-app/multisig/troubleshoot-multisig).
