...
| `global.Pear.run()` spawns workers | [`pear-runtime`](/reference/pear/runtime)'s `run()` method (backed by `bare-sidecar`, which bundles the native Bare runtime) |
...
The build directory layout `pear-runtime` expects is produced by the [`pear build`](https://github.com/holepunchto/hello-pear-electron#deploying) command (Pear v2.5.0+). Your `package.json` needs an `upgrade` field set to the application's `pear://` link — see [Configuration](/reference/pear/configuration#packagejson).
...
| --- | --- |
| `global.Pear.run()` | [`pear-runtime`](/reference/pear/runtime) `run()` |
...
- [`pear provision`](/reference/pear/cli#pear-provision) (Pear v2.6.5+) strips interim operations by block-syncing from a source drive, reducing the disk space an app needs. Use it to create a prerelease.
