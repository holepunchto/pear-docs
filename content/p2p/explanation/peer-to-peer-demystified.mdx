...
<Callout type="info">
This page explains networking concepts. For step-by-step connection guides, see [Connect two peers by key with HyperDHT](/p2p/how-to/connect-to-peers/connect-two-peers-by-key-with-hyperdht) and [Connect to many peers by topic with Hyperswarm](/p2p/how-to/connect-to-peers/connect-to-many-peers-by-topic-with-hyperswarm).
</Callout>
...
As of Pear 3.4.0, that relay path has a concrete CLI surface: [`pear blind-relay start`](/pear/reference/pear/cli#pear-blind-relay) runs a relay host that forwards connections between two peers that cannot hole-punch each other, and [`--relay <key>`](/pear/reference/pear/cli#relay-flag) registers one with the local sidecar, which falls back to it when a connection cannot be made directly. See [Relay connections through a blind relay](/p2p/how-to/connect-to-peers/relay-connections-through-a-blind-relay) for the setup. This is unrelated to [blind peering](/p2p/explanation/blind-peering)'s core replication despite the shared "blind" name—a blind relay forwards connections, it does not store data.
...
<Callout type="info">
  See [Connect two peers by key with HyperDHT](/p2p/how-to/connect-to-peers/connect-two-peers-by-key-with-hyperdht) for a walkthrough of how to use HyperDHT.
</Callout>
...
<Callout type="info">
  [Connect to many peers by topic with Hyperswarm](/p2p/how-to/connect-to-peers/connect-to-many-peers-by-topic-with-hyperswarm) for a walkthrough of how to use Hyperswarm.
</Callout>
...
- [Connect two peers by key with HyperDHT](/p2p/how-to/connect-to-peers/connect-two-peers-by-key-with-hyperdht)—minimal two-peer connection walkthrough.
- [Connect to many peers by topic with Hyperswarm](/p2p/how-to/connect-to-peers/connect-to-many-peers-by-topic-with-hyperswarm)—topic-based discovery and chat.
- [Replicate and persist with Hypercore](/p2p/how-to/store-and-replicate/replicate-and-persist-with-hypercore)—persistence once peers are connected.
- [Dependencies](/pear/explanation/dependencies-and-network)—why Pear uses npm as the install path.
- [From append-only logs to files](/p2p/explanation/from-logs-to-files)—how replicated data is structured above the transport layer.
