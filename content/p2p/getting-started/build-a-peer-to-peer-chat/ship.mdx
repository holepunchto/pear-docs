...
This is **part 3 of 4** in the [getting started path](/p2p/getting-started). You take the production-shaped app from [part 2](/p2p/getting-started/build-a-peer-to-peer-chat/reshape-into-a-production-app) and walk it through the first half of Pear's release pipeline:
...
<Callout type="info">
[Deploy over-the-air updates](/p2p/getting-started/build-a-peer-to-peer-chat/update) (part 4 of 4) continues from here and demonstrates the live OTA cycle on the provision link, plus a tour of multisig.
</Callout>
...
You need:
1. The working production-shaped app from [part 2](/p2p/getting-started/build-a-peer-to-peer-chat/reshape-into-a-production-app).
2. [`pear`](/pear/reference/pear/cli):
...
[`pear touch`](/pear/reference/pear/cli) mints a **new `pear://` link you own**—one backed by a fresh [Hypercore](/p2p/reference/building-blocks/hypercore) whose write key is stored locally on this machine. This becomes your **stage link**: the append-only core you sync builds into before each provision. (The link you set in part 2 was a public placeholder so the app would boot; you replace it now with your own.)
...
[`pear seed`](/pear/reference/pear/cli#pear-seed) keeps that core online so other peers can fetch updates from you.
...
Verlink:       pear://0.128.<stage-key>
App:           pear-chat@1.0.0
Drive Length:  128 [100%]
Blobs Length:  642 [100%]
Blobs Size:    12.4MB
...
[`pear-runtime`](/pear/reference/pear/runtime) only swaps the application drive when the new build advertises a higher version. If you forget this step, peers see your stage and do nothing:
...
The full coverage is in [Build desktop distributables](/pear/how-to/operate-an-app/build-and-package/build-desktop-distributables). See [Desktop release npm scripts](/pear/reference/ci-and-release/desktop-release-npm-scripts) for common `npm` entry points in sample repos.
</Callout>
...
Run these from the **parent directory** of your app (same place you ran `pear build`) unless noted. [`pear stage`](/pear/reference/pear/cli) syncs the deployment directory into the [Hypercore](/p2p/reference/building-blocks/hypercore) behind your **stage link**.
...
<Callout type="info">
The hint blocks at the end of both stage outputs arrive with Pear 3.3.0. On 3.2.0 and earlier the output stops at the versioned link. See [Next-step hints](/pear/reference/pear/cli#next-step-hints).
</Callout>

#### 3. Mint the provision link

Peers should not poll the stage link directly—it keeps the full append-only history. [`pear provision`](/pear/reference/pear/cli#pear-provision) block-syncs the staged snapshot onto a lean **provision link** that packaged apps poll instead. That target is a **second** link—mint it now with another `pear touch` (the first touch in step 1 was only for staging). Cwd does not matter; `pear touch` does not read your project:
...
The diff should mirror what you just staged. The `pear seed` line in the hint block at the end names the unversioned provision link peers will poll—set `upgrade` to that link in the next step, not the versioned stage link.

<Callout type="info">
On Pear 3.2.0 and earlier the same link is printed inside the `Provisioned:` block under a `Seed with:` heading instead of in a hint block. See [Next-step hints](/pear/reference/pear/cli#next-step-hints).
</Callout>
...
<Callout type="info">
Keep both links: **stage** for `pear stage` on every iteration; **provision** for what apps install and poll. Part 4 repeats `pear stage` → `pear provision` for v1.0.2. The full operator reference is [Deploy your application—provision](/pear/how-to/operate-an-app/manual-deployment/deployment#6-provision).
</Callout>
...
| `pear stage` | Append-only sync into the stage [Hypercore](/p2p/reference/building-blocks/hypercore) | History is permanent; updates are not |
...
- **Continue the path:** [Deploy over-the-air updates](/p2p/getting-started/build-a-peer-to-peer-chat/update) (part 4 of 4)—run the installed build, ship a second version, watch OTA fire end-to-end, and preview multisig.
- **Automate it:** [Publish with GitHub Actions](/pear/how-to/operate-an-app/github-actions/publish-with-github-actions)—skip the manual staging and let CI stage a stable `pear://` link on every push.
...
- [Build desktop distributables](/pear/how-to/operate-an-app/build-and-package/build-desktop-distributables)—code-signing, notarization, MSIX publisher details.
- [Publish a changelog for your app](/pear/how-to/operate-an-app/publish-a-changelog)—ship a `CHANGELOG.md` so users read your release notes with `pear changelog`.
- [Troubleshoot desktop releases](/pear/how-to/operate-an-app/manual-deployment/troubleshoot-desktop-releases)—"the app did not update," lost write-access, stage size blowups.
