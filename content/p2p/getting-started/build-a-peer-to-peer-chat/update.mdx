...
This is **part 4 of 4** in the [getting started path](/p2p/getting-started). You start from the v1.0.1 you shipped in [part 3](/p2p/getting-started/build-a-peer-to-peer-chat/ship):

- a [**provision link**](/p2p/getting-started/build-a-peer-to-peer-chat/ship#5-switch-upgrade-to-the-provision-link) in `package.json#upgrade`
- an active [`pear seed`](/p2p/getting-started/build-a-peer-to-peer-chat/ship#5-switch-upgrade-to-the-provision-link) on that link
- a [packaged build](/p2p/getting-started/build-a-peer-to-peer-chat/ship#6-rebuild-once) that embeds the same field.
...
You need:
1. The shipped v1.0.1 from [part 3—ship](/p2p/getting-started/build-a-peer-to-peer-chat/ship). That part covered `pear touch`, the stage and provision links, `npm version patch`, `electron-forge make`, `pear build`, `pear stage`, and `pear provision`.
...
Use the versioned link from the `pear stage` output for the first argument. `<provision-key>` is the key segment from `<provision-link>` (everything after `pear://`), same as in [part 3](/p2p/getting-started/build-a-peer-to-peer-chat/ship#stage-and-provision-the-first-version).
