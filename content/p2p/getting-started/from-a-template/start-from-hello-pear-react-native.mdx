...
<Callout type="info">
**OTA differs from desktop.** A mobile OTA payload is a JavaScript bundle per host, not a set of native distributables, and native/OTA releases share one SemVer sequence gated by `pear.json`'s `updates.minver`—see [Pear Mobile OTA](/pear/reference/pear/mobile#the-minver-gate). The full staging, provisioning, and multisig ceremony is documented in the upstream [`hello-pear-react-native` README](https://github.com/holepunchto/hello-pear-react-native#deployments)—it reuses [Deploy your application](/pear/how-to/operate-an-app/manual-deployment/deployment)'s model but with a different build step, so that page's distributables flow doesn't apply here verbatim.
</Callout>
