...
`npm run make` compiles `bin.mjs` and its dependencies into a single standalone executable with no peer dependencies—users don't need Node.js, Bare, or the [Pear CLI](/pear/reference/pear/cli) installed. `scripts/make.js` detects your OS and architecture and runs the matching target, writing the binary to `out/<platform>-<arch>`; the template ships a target for every supported platform (macOS, Linux, and Windows, each for `arm64`/`x64`). Build each platform's binary on a matching host.
