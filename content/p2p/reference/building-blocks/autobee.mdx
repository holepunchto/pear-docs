...
<Callout type="warn">
Upstream describes Autobee as **experimental and under heavy development, with breaking changes expected**. Pin an exact version, and re-read this page against your installed release before upgrading. It is documented here against **v2.9.5**.
</Callout>
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L32)
...
| `ackThreshold` | `32` | Flushes-behind threshold below which acking is skipped, unless overridden per call via [`db.setAcking()`](#dbsetackingacking-options)'s own `threshold`. |
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L67)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L68)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L69)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L87)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L172)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L168)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L82)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L360)

- Returns: `object` — the current system and view core positions.

#### `db.bee`

[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L81)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L72)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L83)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L209)

Override acking, which is otherwise driven by the `isTrusted` constructor option. `options.threshold` defaults to the current threshold.

| Parameter | Description |
| --- | --- |
| `acking` | The new acking state. |
| `options?` | `{ threshold }` — override the ack threshold too. |

#### `db.encrypted`

[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L48)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L1329)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L610)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L614)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L412)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L406)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L1323)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L416)

Queue a wakeup hint, or an array of them, without awaiting a cycle.

#### `const cores = await db.cores(options)`

[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L271)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L1471)

Fast-forward onto `head`, skipping the usual distance and conservative checks.

- Returns: [`Promise<object|null>`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise) — resolves to `{ to, from }`, or `null` if `head` could not be booted.

#### `db.getMostRecentHead()`

[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L440)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L913)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L164)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L1317)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L1313)

Decode an Autobee block back to its value.

#### `Autobee.GENESIS`

[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L162)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L638)

Emitted after an interrupt, alongside `interrupt`, and separately whenever the local writer's writable state flips (see [`db.on('writable', ...)`](#dbonwritable-----) below) — [src](https://github.com/holepunchto/autobee/blob/v2.9.5/lib/writers.js#L189). It is not a general view-changed signal for an ordinary apply cycle; use the `update(view, changes)` handler option for that.

#### `db.on('writable', () => { ... })`

[src](https://github.com/holepunchto/autobee/blob/v2.9.5/lib/writers.js#L188)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L637)

Emitted when [`host.interrupt(reason)`](#hostinterruptreason) is called inside `apply`.

#### `db.on('error', (err) => { ... })`

[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L651)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L961)

Emitted after [`db.setLocal()`](#await-dbsetlocalkey-options) swaps the local writer.

#### `db.on('move-to', (to, from) => { ... })`

[src](https://github.com/holepunchto/autobee/blob/v2.9.5/index.js#L1600)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/lib/apply-calls.js#L33)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/lib/apply-calls.js#L49)

Remove a writer.

#### `host.ackWriter(key)`

[src](https://github.com/holepunchto/autobee/blob/v2.9.5/lib/apply-calls.js#L44)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/lib/apply-calls.js#L58)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/lib/apply-calls.js#L66)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/lib/apply-calls.js#L29)
...
[src](https://github.com/holepunchto/autobee/blob/v2.9.5/lib/apply-calls.js#L25)
...
<Callout type="info">
Two host calls are stubs in v2.9.5 and should not be relied on: `host.removeable(key)` always returns `true`, and `host.preferFastForward()` does nothing.
</Callout>
...
| `incremental` | `boolean` | Whether the change extended the previous state rather than replacing it. Always `false` in v2.9.5 — `track()` hardcodes it and nothing reassigns it, so this field carries no information yet. |
