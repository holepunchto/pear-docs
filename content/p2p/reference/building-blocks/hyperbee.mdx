...
`Hyperbee` stores sorted key/value entries in an append-only B-tree backed by [`Hypercore`](/p2p/reference/building-blocks/hypercore). Use it when you need ordered lookups, range scans, atomic batches, namespaces, and point-in-time snapshots over one replicated log.
...
| --- | --- | --- |
| `core` | [`Hypercore`](/p2p/reference/building-blocks/hypercore) | — |
...
- Returns: [`Batch`](/p2p/reference/building-blocks/hyperbee) — A new batch; queue writes on it and call `flush()` to commit.
...
| Parameter | Type | Description |
| --- | --- | --- |
| `right` | [`number\|Hyperbee`](/p2p/reference/building-blocks/hyperbee) | The other version to diff against: a version number, or another bee/snapshot. |
...
- Returns: [`Promise<EntryWatcher>`](/p2p/reference/building-blocks/hyperbee) — a watcher which listens to changes on the given key.
...
- Returns: [`Watcher`](/p2p/reference/building-blocks/hyperbee) — a new value after a change, `current` and `previous` are snapshots that are auto-closed before next value.
...
- Returns: [`Hyperbee`](/p2p/reference/building-blocks/hyperbee) — A read-only bee pinned to that version.
...
- Returns: [`Hyperbee`](/p2p/reference/building-blocks/hyperbee) — A read-only bee pinned to the current version.
...
- Returns: [`Hyperbee`](/p2p/reference/building-blocks/hyperbee) — A bee scoped to the prefixed keyspace.
...
| Parameter | Type | Description |
| --- | --- | --- |
| `core` | [`Hypercore`](/p2p/reference/building-blocks/hypercore) | The core to inspect. |
...
| --- | --- | --- | --- |
| `seq` | `number` | — | [Hypercore](/p2p/reference/building-blocks/hypercore) index (block number) at which this entry was inserted. |
...
| `prefix` | `Buffer` | — | Namespace prefix applied to every key; set internally by [`db.sub()`](#dbsubsub-prefix-options--) rather than passed directly in most apps. |
| `sep` | `Buffer\|string` | — | Separator placed between a `prefix` and each key. |
| `readonly` | `boolean` | `false` | Open the bee without acquiring the write lock. |
| `checkout` | `number` | `0` | Open the bee already checked out at this version, equivalent to calling [`db.checkout()`](#dbcheckoutversion) immediately after construction. |
| `lock` | `object` | — | A [`mutexify`](https://github.com/mafintosh/mutexify) instance to coordinate writes with; shared automatically between a bee and its subs. |
| `metadata` | `*` | — | Arbitrary value stored in the bee's header, returned by [`db.getHeader()`](#await-dbgetheaderoptions). |
| `extension` | `boolean\|object` | — | The [Protomux](/p2p/reference/helpers/protomux) extension used for range-watch notifications between peers; pass `false` to disable it. |
| `alwaysDuplicate` | `boolean` | `true` | Duplicate key/value buffers read from storage rather than returning views into the underlying block. Planned to default to `false` in a future version. |
...
- [Autobee](/p2p/reference/building-blocks/autobee)—the multiwriter form of this B-tree, where many peers write into one derived view.
