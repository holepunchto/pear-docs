...
<Callout type="info">
If `onmessage`/`onopen`/`onclose`/`ondrain`/`ondestroy` throws after its channel has already closed, the underlying stream emits a `warning` event with the error instead of destroying the stream (added in v3.12.0). While the channel is still open, a thrown handler error still destroys the stream as before — listen for `warning` on the stream if you rely on catching post-close handler errors.
</Callout>
