...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L331)
...
```js
{
  // Called when the muxer wants to allocate a message that is written.
  // Defaults to the stream's own alloc method when it has one, otherwise Buffer.allocUnsafe.
  alloc (size) {}
}
```
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L365)
...
- Returns: [`Protomux`](/p2p/reference/helpers/protomux) — The existing or newly created Protomux instance.
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L371)
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L385)
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L392)
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L381)

Convenience method that returns true when no channels are open and no closed channel is still awaiting the remote's acknowledgement.

- Returns: `boolean` — `true` when the muxer has no open channels and none awaiting a remote close acknowledgement.
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L407)
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L411)
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L415)
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L400)

Returns the channel that most recently opened. Returns `null` once that channel closes, even if older channels with the same protocol and id are still open.

| Parameter | Type | Description |
| --- | --- | --- |
| `options` | [`ChannelKey`](#channelkey) | Protocol name and optional binary id. |

- Returns: [`Channel\|null`](/p2p/reference/helpers/protomux) — The most recently opened matching channel, or `null`.
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L421)
...
- Returns: [`Channel\|null`](/p2p/reference/helpers/protomux) — The new channel, or `null` if it cannot be opened.

### Mux properties

#### `mux.stream`

[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L335)
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L337)
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L242)
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L234)
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L238)
...
[src](https://github.com/holepunchto/protomux/blob/v3.12.0/index.js#L259)
...
<Callout type="info">
If an async `onmessage`/`onopen`/`onclose`/`ondrain`/`ondestroy` handler returns a promise that rejects after its channel has already closed, the underlying stream emits a `warning` event with the error instead of destroying the stream (added in v3.12.0). While the channel is still open, a rejection still destroys the stream as before, and a handler that throws synchronously is not caught here at all — listen for `warning` on the stream if you rely on catching post-close handler errors.
</Callout>
...
| --- | --- | --- | --- |
| `alloc` | `Function` | — | Custom allocator; called with `(size)` and must return a `Buffer`. Defaults to the stream's own `alloc` method when it has one, otherwise `Buffer.allocUnsafe`. |
...
| `id` | [`Buffer\|null`](https://nodejs.org/api/buffer.html#class-buffer) | `null` | Optional binary id, combined with `protocol` into an exact key. `opened` and `getLastChannel` only match channels created with the same id; a protocol-only `pair` registration matches channels with any id. |
