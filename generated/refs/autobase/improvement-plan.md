...
Generated from `holepunchto/autobase` at **v7.28.2** (`f28f3424d2`) on 2026-09-17T14:15:49.635Z.
**Doc-completeness: 62%** — 18 of 29 source methods fully documented (description + documented params). Return values and examples are reported separately as enhancement signals.
...
- `const base = new Autobase(store, bootstrap, opts)` → store, bootstrap, handlers
- `const stream = base.replicate(isInitiator || stream, opts)` → isInitiator, opts
- `base.setBigBatches(enable = true)` → bool
...
- `await base.setLocal(key, options = {})`
...
- `wakeupHandler.onpeeractive(peer, session)`
- `wakeupHandler.onlookup(req, peer, session)`
- `wakeupHandler.onannounce(wakeup, peer, session)`
- `wakeupHandler.active`
- `wakeupHandler.discoveryKey`
- `wakeupHandler.base`

### Stale README (not found in source) (6)
