...
Sends an mDNS query for `name`. `type` defaults to `TYPE.PTR`.
...
Queries for the configured service every 2 seconds until `timeout` milliseconds elapse (default `10000`), then resolves with all services found. If `opts.first` is set, resolves as soon as the first service is found instead of waiting for the timeout.
