...
upstreamVersion: "4.8.1"
---

<mark style={{ backgroundColor: '#7dde9a', padding: '2px 8px', borderRadius: '4px', fontSize: '0.85em', fontWeight: 500 }}>stable</mark>

[`bare-fs`](https://github.com/holepunchto/bare-fs) — Native file system operations for Bare. It is a native addon and requires Bare `>=1.28.0`.
...
- `ENOENT` — `filepath` does not exist and `flags` does not include a creating variant (for
example the default `'r'`).
- `EEXIST` — `flags` is an exclusive variant (`'wx'`, `'ax'`, `'xw'`, `'xa'`, etc.) and
...
- `EEXIST` — `filepath` already exists; when `opts.recursive` is set this is only thrown if
...
#### `ReadStream.flags: Flag`

The flags the file was opened with.

#### `ReadStream.mode: number`

The file mode (type and permissions).

#### `ReadStream.path: string | null`
...
**Returns** `unknown` | `null` — The number of bytes actually read, which may be less than `len` (`0` at end of file).
...
#### `WriteStream.flags: Flag`

The flags the file was opened with.

#### `WriteStream.mode: number`

The file mode (type and permissions).

#### `WriteStream.path: string | null`
...
| Parameter | Type | Default | Description |
| --- | --- | --- | --- |
| `data` | `unknown` | — | The bytes to write. May also be a string, in which case the signature becomes `fs.write(fd, data[, pos[, encoding]])`. |
| `encoding?` | `BufferEncoding` | — | — |
| `cb?` | `StreamCallback` | — | — |

**Returns** `boolean` — The number of bytes actually written, which may be less than `data`'s length.
...
- Builds on `bare-events`, `bare-path`, `bare-stream`, and `bare-url`.
