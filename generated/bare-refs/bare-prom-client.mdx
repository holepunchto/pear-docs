---
title: "bare-prom-client"
description: "Client for prometheus"
docType: reference
schemaType: APIReference
---

<mark style={{ backgroundColor: '#7dde9a', padding: '2px 8px', borderRadius: '4px', fontSize: '0.85em', fontWeight: 500 }}>stable</mark>

[`bare-prom-client`](https://github.com/holepunchto/bare-prom-client) — Client for prometheus.

```sh
npm i bare-prom-client
```

## Usage

See example folder for a sample usage. The library does not bundle any web
framework. To expose the metrics, respond to Prometheus's scrape requests with
the result of `await registry.metrics()`.

### Usage with Node.js's `cluster` module

Node.js's `cluster` module spawns multiple processes and hands off socket
connections to those workers. Returning metrics from a worker's local registry
will only reveal that individual worker's metrics, which is generally
undesirable. To solve this, you can aggregate all of the workers' metrics in the
master process. See `example/cluster.js` for an example.

Default metrics use sensible aggregation methods. (Note, however, that the event
loop lag mean and percentiles are averaged, which is not perfectly accurate.)
Custom metrics are summed across workers by default. To use a different
aggregation method, set the `aggregator` property in the metric config to one of
'sum', 'first', 'min', 'max', 'average' or 'omit'. (See `lib/metrics/version.js`
for an example.)

If you need to expose metrics about an individual worker, you can include a
value that is unique to the worker (such as the worker ID or process ID) in a
label. (See `example/server.js` for an example using
`worker_${cluster.worker.id}` as a label value.)

Metrics are aggregated from the global registry by default. To use a different
registry, call
`client.AggregatorRegistry.setRegistries(registryOrArrayOfRegistries)` from the
worker processes.

## API

### Registry

#### `clear(): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L35)</sub>

Remove all metrics from the registry

#### `contentType: BoundRegistryContentType`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L86)</sub>

Gets the Content-Type of the metrics for use in the response headers.

#### `getMetricsAsArray(): MetricObject[]`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L56)</sub>

Get all metrics as objects

#### `getMetricsAsJSON(): Promise<MetricObjectWithValues<MetricValue<string>>[]>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L51)</sub>

Get all metrics as objects

#### `getSingleMetric<T extends string>(name: string): Metric<T> | undefined`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L68)</sub>

Get a single metric

**Parameters**

- `name` (`string`) — The name of the metric

#### `getSingleMetricAsString(name: string): Promise<string>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L81)</sub>

Get a string representation of a single metric by name

**Parameters**

- `name` (`string`) — The name of the metric

#### `metrics(): Promise<string>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L30)</sub>

Get string representation for all metrics

#### `registerMetric<T extends string>(metric: Metric<T>): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L46)</sub>

Register metric to register

**Parameters**

- `metric` (`Metric<T>`) — Metric to add to register

#### `Registry.merge(registers: Registry[]): Registry`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L99)</sub>

Merge registers

**Parameters**

- `registers` (`Registry[]`) — The registers you want to merge together

#### `Registry.OPENMETRICS_CONTENT_TYPE: OpenMetricsContentType`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L109)</sub>

HTTP OpenMetrics Content-Type for metrics response headers.

#### `Registry.PROMETHEUS_CONTENT_TYPE: PrometheusContentType`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L104)</sub>

HTTP Prometheus Content-Type for metrics response headers.

#### `removeSingleMetric(name: string): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L62)</sub>

Remove a single metric

**Parameters**

- `name` (`string`) — The name of the metric to remove

#### `resetMetrics(): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L40)</sub>

Reset all metrics in the registry

#### `setContentType(contentType: BoundRegistryContentType): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L93)</sub>

Set the content type of a registry. Used to change between Prometheus and
OpenMetrics versions.

**Parameters**

- `contentType` (`BoundRegistryContentType`) — The type of the registry

#### `setDefaultLabels(labels: object): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L75)</sub>

Set static labels to every metric emitted by this registry

**Parameters**

- `labels` (`object`) — of name/value pairs:
\{ defaultLabel: "value", anotherLabel: "value 2" \}

### AggregatorRegistry

#### `AggregatorRegistry.aggregate`

```ts
AggregatorRegistry.aggregate<T extends RegistryContentType>(metricsArr: Array<object>): Registry<T>
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L153)</sub>

Creates a new Registry instance from an array of metrics that were
created by `registry.getMetricsAsJSON()`. Metrics are aggregated using
the method specified by their `aggregator` property, or by summation if
`aggregator` is undefined.

**Parameters**

- `metricsArr` (`Array<object>`) — Array of metrics, each of which created by
`registry.getMetricsAsJSON()`.

**Returns** `Registry<T>` — aggregated registry.

#### `AggregatorRegistry.setRegistries`

```ts
AggregatorRegistry.setRegistries(regs: | Array<
					Registry<PrometheusContentType> | Registry<OpenMetricsContentType>
			  >
			| Registry<PrometheusContentType>
			| Registry<OpenMetricsContentType>): void
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L164)</sub>

Sets the registry or registries to be aggregated. Call from workers to
use a registry/registries other than the default global registry.

**Parameters**

- `regs` (`| Array< Registry<PrometheusContentType> | Registry<OpenMetricsContentType> > | Registry<PrometheusContentType> | Registry<OpenMetricsContentType>`) — Registry or registries to be
aggregated.

#### `clusterMetrics(): Promise<string>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L142)</sub>

Gets aggregated metrics for all workers.

**Returns** `Promise<string>` — Promise that resolves with the aggregated
metrics.

### Counter

#### `new Counter(configuration: CounterConfiguration<T>)`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L258)</sub>

Counters go up, and reset when the process restarts.

**Parameters**

- `configuration` (`CounterConfiguration<T>`) — Configuration when creating a Counter metric. Name and Help is required.

#### `Counter.Internal`

```ts
interface Internal {
		/**
		 * Increment with value
		 * @param value The value to increment with
		 */
		inc(value?: number): void;
	}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L317)</sub>

#### `Counter.get(): Promise<MetricObjectWithValues<MetricValue<T>>>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L282)</sub>

Get counter metric object

#### `Counter.inc(labels: LabelValues<T>, value?: number): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L265)</sub>

Increment for given labels

Overloads:

```ts
inc(labels: LabelValues<T>, value?: number): void
inc(value?: number): void
inc(incData: IncreaseDataWithExemplar<T>): void
```

**Parameters**

- `labels` (`LabelValues<T>`) — Object with label keys and values
- `value` (`number`, optional) — The number to increment with

#### `labels(...values: string[]): Counter.Internal`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L289)</sub>

Return the child for given labels

Overloads:

```ts
labels(...values: string[]): Counter.Internal
labels(labels: LabelValues<T>): Counter.Internal
```

**Parameters**

- `values` (`string[]`) — Label values

**Returns** `Counter.Internal` — Configured counter with given labels

#### `Counter.remove(...values: string[]): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L307)</sub>

Remove metrics for the given label values

Overloads:

```ts
remove(...values: string[]): void
remove(labels: LabelValues<T>): void
```

**Parameters**

- `values` (`string[]`) — Label values

#### `Counter.reset(): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L301)</sub>

Reset counter values

### Gauge

#### `new Gauge(configuration: GaugeConfiguration<T>)`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L338)</sub>

Gauges are similar to Counters but a Gauge's value can be decreased.

**Parameters**

- `configuration` (`GaugeConfiguration<T>`) — Configuration when creating a Gauge metric. Name and Help is mandatory

#### `dec(labels: LabelValues<T>, value?: number): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L358)</sub>

Decrement gauge

Overloads:

```ts
dec(labels: LabelValues<T>, value?: number): void
dec(value?: number): void
```

**Parameters**

- `labels` (`LabelValues<T>`) — Object with label keys and values
- `value` (`number`, optional) — Value to decrement with

#### `Gauge.Internal`

```ts
interface Internal<T extends string> {
		/**
		 * Increment gauge with value
		 * @param value The value to increment with
		 */
		inc(value?: number): void;

		/**
		 * Decrement with value
		 * @param value The value to decrement with
		 */
		dec(value?: number): void;

		/**
		 * Set gauges value
		 * @param value The value to set
		 */
		set(value: number): void;

		/**
		 * Set gauge value to current epoch time in ms
		 */
		setToCurrentTime(): void;

		/**
		 * Start a timer. Calling the returned function will set the gauge's value
		 * to the observed duration in seconds.
		 * @return Function to invoke when timer should be stopped. The value it
		 * returns is the timed duration.
		 */
		startTimer(): (labels?: LabelValues<T>) => number;
	}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L432)</sub>

#### `Gauge.get(): Promise<MetricObjectWithValues<MetricValue<T>>>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L382)</sub>

Get gauge metric object

#### `Gauge.inc(labels: LabelValues<T>, value?: number): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L345)</sub>

Increment gauge for given labels

Overloads:

```ts
inc(labels: LabelValues<T>, value?: number): void
inc(value?: number): void
```

**Parameters**

- `labels` (`LabelValues<T>`) — Object with label keys and values
- `value` (`number`, optional) — The value to increment with

#### `labels(...values: string[]): Gauge.Internal<T>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L404)</sub>

Return the child for given labels

Overloads:

```ts
labels(...values: string[]): Gauge.Internal<T>
labels(labels: LabelValues<T>): Gauge.Internal<T>
```

**Parameters**

- `values` (`string[]`) — Label values

**Returns** `Gauge.Internal<T>` — Configured gauge with given labels

#### `Gauge.remove(...values: string[]): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L422)</sub>

Remove metrics for the given label values

Overloads:

```ts
remove(...values: string[]): void
remove(labels: LabelValues<T>): void
```

**Parameters**

- `values` (`string[]`) — Label values

#### `Gauge.reset(): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L416)</sub>

Reset gauge values

#### `set(labels: LabelValues<T>, value: number): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L371)</sub>

Set gauge value for labels

Overloads:

```ts
set(labels: LabelValues<T>, value: number): void
set(value: number): void
```

**Parameters**

- `labels` (`LabelValues<T>`) — Object with label keys and values
- `value` (`number`) — The value to set

#### `setToCurrentTime(labels?: LabelValues<T>): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L388)</sub>

Set gauge value to current epoch time in seconds

**Parameters**

- `labels` (`LabelValues<T>`, optional) — Object with label keys and values

#### `Gauge.startTimer(labels?: LabelValues<T>): (labels?: LabelValues<T>) => number`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L397)</sub>

Start a timer. Calling the returned function will set the gauge's value
to the observed duration in seconds.

**Parameters**

- `labels` (`LabelValues<T>`, optional) — Object with label keys and values

**Returns** `(labels?: LabelValues<T>) => number` — Function to invoke when timer should be stopped. The value it
returns is the timed duration.

### Histogram

#### `new Histogram(configuration: HistogramConfiguration<T>)`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L479)</sub>

Histograms track sizes and frequency of events.

**Parameters**

- `configuration` (`HistogramConfiguration<T>`) — Configuration when creating the Histogram. Name and Help is mandatory

#### `Histogram.get(): Promise<MetricObjectWithValues<MetricValueWithName<T>>>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L502)</sub>

Get histogram metric object

#### `Histogram.Config`

```ts
interface Config {
		/**
		 * Buckets used in the histogram
		 */
		buckets?: number[];
	}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L581)</sub>

#### `Histogram.Internal`

```ts
interface Internal<T extends string> {
		/**
		 * Observe value
		 * @param value The value to observe
		 */
		observe(value: number): void;

		/**
		 * Start a timer. Calling the returned function will observe the
		 * duration in seconds in the histogram.
		 * @param labels Object with label keys and values
		 * @return Function to invoke when timer should be stopped. The value it
		 * returns is the timed duration.
		 */
		startTimer(): (labels?: LabelValues<T>) => void;
	}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L564)</sub>

#### `labels(...values: string[]): Histogram.Internal<T>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L541)</sub>

Return the child for given labels

Overloads:

```ts
labels(...values: string[]): Histogram.Internal<T>
labels(labels: LabelValues<T>): Histogram.Internal<T>
```

**Parameters**

- `values` (`string[]`) — Label values

**Returns** `Histogram.Internal<T>` — Configured histogram with given labels

#### `Histogram.observe(value: number): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L485)</sub>

Observe value

Overloads:

```ts
observe(value: number): void
observe(labels: LabelValues<T>, value: number): void
observe(observeData: ObserveDataWithExemplar<T>): void
```

**Parameters**

- `value` (`number`) — The value to observe

#### `Histogram.remove(...values: string[]): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L554)</sub>

Remove metrics for the given label values

Overloads:

```ts
remove(...values: string[]): void
remove(labels: LabelValues<T>): void
```

**Parameters**

- `values` (`string[]`) — Label values

#### `Histogram.reset(): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L529)</sub>

Reset histogram values

#### `Histogram.startTimer(labels?: LabelValues<T>): (labels?: LabelValues<T>) => number`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L511)</sub>

Start a timer. Calling the returned function will observe the duration in
seconds in the histogram.

Overloads:

```ts
startTimer(labels?: LabelValues<T>): (labels?: LabelValues<T>) => number
startTimer(labels?: LabelValues<T>, exemplarLabels?: LabelValues<T>): (labels?: LabelValues<T>, exemplarLabels?: LabelValues<T>) => number
```

**Parameters**

- `labels` (`LabelValues<T>`, optional) — Object with label keys and values

**Returns** `(labels?: LabelValues<T>) => number` — Function to invoke when timer should be stopped. The value it
returns is the timed duration.

#### `zero(labels: LabelValues<T>): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L534)</sub>

Initialize the metrics for the given combination of labels to zero

**Parameters**

- `labels` (`LabelValues<T>`)

### Summary

#### `new Summary(configuration: SummaryConfiguration<T>)`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L606)</sub>

Summaries calculate percentiles of observed values.

**Parameters**

- `configuration` (`SummaryConfiguration<T>`) — Configuration when creating Summary metric. Name and Help is mandatory

#### `Summary.get(): Promise<MetricObjectWithValues<MetricValueWithName<T>>>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L623)</sub>

Get summary metric object

#### `labels(...values: string[]): Summary.Internal<T>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L643)</sub>

Return the child for given labels

Overloads:

```ts
labels(...values: string[]): Summary.Internal<T>
labels(labels: LabelValues<T>): Summary.Internal<T>
```

**Parameters**

- `values` (`string[]`) — Label values

**Returns** `Summary.Internal<T>` — Configured summary with given labels

#### `Summary.observe(value: number): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L612)</sub>

Observe value in summary

Overloads:

```ts
observe(value: number): void
observe(labels: LabelValues<T>, value: number): void
```

**Parameters**

- `value` (`number`) — The value to observe

#### `Summary.remove(...values: string[]): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L656)</sub>

Remove metrics for the given label values

Overloads:

```ts
remove(...values: string[]): void
remove(labels: LabelValues<T>): void
```

**Parameters**

- `values` (`string[]`) — Label values

#### `Summary.reset(): void`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L636)</sub>

Reset all values in the summary

#### `Summary.startTimer(labels?: LabelValues<T>): (labels?: LabelValues<T>) => number`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L631)</sub>

Start a timer. Calling the returned function will observe the duration in
seconds in the summary.

**Parameters**

- `labels` (`LabelValues<T>`, optional) — Object with label keys and values

**Returns** `(labels?: LabelValues<T>) => number` — Function to invoke when timer should be stopped

#### `Summary.Config`

```ts
interface Config {
		/**
		 * Configurable percentiles, values should never be greater than 1
		 */
		percentiles?: number[];
	}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L683)</sub>

#### `Summary.Internal`

```ts
interface Internal<T extends string> {
		/**
		 * Observe value in summary
		 * @param value The value to observe
		 */
		observe(value: number): void;

		/**
		 * Start a timer. Calling the returned function will observe the
		 * duration in seconds in the summary.
		 * @param labels Object with label keys and values
		 * @return Function to invoke when timer should be stopped. The value it
		 * returns is the timed duration.
		 */
		startTimer(): (labels?: LabelValues<T>) => number;
	}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L666)</sub>

### Pushgateway

#### `new Pushgateway(url: string, options?: any, registry?: Registry<T>)`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L700)</sub>

**Parameters**

- `url` (`string`) — Complete url to the Pushgateway. If port is needed append url with :port
- `options` (`any`, optional) — Options
- `registry` (`Registry<T>`, optional) — Registry

#### `delete(params: Pushgateway.Parameters): Promise<{ resp?: unknown; body?: unknown }>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L722)</sub>

Delete all metrics for jobName

**Parameters**

- `params` (`Pushgateway.Parameters`) — Push parameters

#### `push(params: Pushgateway.Parameters): Promise<{ resp?: unknown; body?: unknown }>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L714)</sub>

Overwrite all metric (using PUT to Pushgateway)

**Parameters**

- `params` (`Pushgateway.Parameters`) — Push parameters

#### `pushAdd(params: Pushgateway.Parameters): Promise<{ resp?: unknown; body?: unknown }>`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L706)</sub>

Add metric and overwrite old ones

**Parameters**

- `params` (`Pushgateway.Parameters`) — Push parameters

#### `Pushgateway.Parameters`

```ts
interface Parameters {
		/**
		 * Jobname that is pushing the metric
		 */
		jobName: string;
		/**
		 * Label sets used in the url when making a request to the Pushgateway,
		 */
		groupings?: {
			[key: string]: string;
		};
	}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L728)</sub>

### Functions

#### `linearBuckets(start: number, width: number, count: number): number[]`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L749)</sub>

Create an array with equal spacing between the elements

**Parameters**

- `start` (`number`) — The first value in the array
- `width` (`number`) — The spacing between the elements
- `count` (`number`) — The number of items in array

**Returns** `number[]` — An array with the requested number of elements

#### `exponentialBuckets(start: number, factor: number, count: number): number[]`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L762)</sub>

Create an array that grows exponentially

**Parameters**

- `start` (`number`) — The first value in the array
- `factor` (`number`) — The exponential factor
- `count` (`number`) — The number of items in array

**Returns** `number[]` — An array with the requested number of elements

#### `validateMetricName(name: string): boolean`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L795)</sub>

Validate a metric name

**Parameters**

- `name` (`string`) — The name to validate

**Returns** `boolean` — True if the metric name is valid, false if not

### Constants and variables

#### `register: Registry`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L116)</sub>

The register that contains all metrics

#### `contentType: RegistryContentType`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L122)</sub>

HTTP Content-Type for metrics response headers for the default registry,
defaults to Prometheus text format.

#### `prometheusContentType: PrometheusContentType`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L127)</sub>

HTTP Prometheus Content-Type for metrics response headers.

#### `openMetricsContentType: OpenMetricsContentType`

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L132)</sub>

HTTP OpenMetrics Content-Type for metrics response headers.

#### `MetricType`

```ts
enum MetricType {
	Counter,
	Gauge,
	Histogram,
	Summary,
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L188)</sub>

#### `collectDefaultMetrics`

```ts
collectDefaultMetrics: {
	/**
	 * Configure default metrics
	 * @param config Configuration object for default metrics collector
	 */
	<T extends RegistryContentType>(
		config?: DefaultMetricsCollectorConfiguration<T>,
	): void;
	/** All available default metrics */
	metricsList: string[];
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L778)</sub>

### Types

#### `Charset`

```ts
type Charset = 'utf-8'
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L4)</sub>

#### `PrometheusMIME`

```ts
type PrometheusMIME = 'text/plain'
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L6)</sub>

#### `PrometheusMetricsVersion`

```ts
type PrometheusMetricsVersion = '0.0.4'
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L7)</sub>

#### `OpenMetricsMIME`

```ts
type OpenMetricsMIME = 'application/openmetrics-text'
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L9)</sub>

#### `OpenMetricsVersion`

```ts
type OpenMetricsVersion = '1.0.0'
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L10)</sub>

#### `OpenMetricsContentType`

```ts
type OpenMetricsContentType = `${OpenMetricsMIME}; version=${OpenMetricsVersion}; charset=${Charset}`
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L12)</sub>

#### `PrometheusContentType`

```ts
type PrometheusContentType = `${PrometheusMIME}; version=${PrometheusMetricsVersion}; charset=${Charset}`
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L14)</sub>

#### `RegistryContentType`

```ts
type RegistryContentType = | PrometheusContentType
	| OpenMetricsContentType
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L17)</sub>

#### `Collector`

```ts
type Collector = () => void
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L111)</sub>

#### `Metric`

```ts
type Metric<T extends string = string> = | Counter<T>
	| Gauge<T>
	| Summary<T>
	| Histogram<T>
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L177)</sub>

General metric type

#### `Aggregator`

```ts
type Aggregator = 'omit' | 'sum' | 'first' | 'min' | 'max' | 'average'
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L186)</sub>

Aggregation methods, used for aggregating metrics in a Node.js cluster.

#### `CollectFunction`

```ts
type CollectFunction<T> = (this: T) => void | Promise<void>
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L195)</sub>

#### `MetricObject`

```ts
interface MetricObject {
	name: string;
	help: string;
	type: MetricType;
	aggregator: Aggregator;
	collect: CollectFunction<any>;
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L197)</sub>

#### `MetricObjectWithValues`

```ts
interface MetricObjectWithValues<T extends MetricValue<string>>
	extends MetricObject {
	values: T[];
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L205)</sub>

#### `MetricValue`

```ts
type MetricValue<T extends string> = {
	value: number;
	labels: LabelValues<T>;
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L210)</sub>

#### `MetricValueWithName`

```ts
type MetricValueWithName<T extends string> = MetricValue<T> & {
	metricName?: string;
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L215)</sub>

#### `LabelValues`

```ts
type LabelValues<T extends string> = Partial<Record<T, string | number>>
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L219)</sub>

#### `MetricConfiguration`

```ts
interface MetricConfiguration<T extends string> {
	name: string;
	help: string;
	labelNames?: T[] | readonly T[];
	registers?: (
		| Registry<PrometheusContentType>
		| Registry<OpenMetricsContentType>
	)[];
	aggregator?: Aggregator;
	collect?: CollectFunction<any>;
	enableExemplars?: boolean;
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L221)</sub>

#### `CounterConfiguration`

```ts
interface CounterConfiguration<T extends string>
	extends MetricConfiguration<T> {
	collect?: CollectFunction<Counter<T>>;
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L234)</sub>

#### `IncreaseDataWithExemplar`

```ts
interface IncreaseDataWithExemplar<T extends string> {
	value?: number;
	labels?: LabelValues<T>;
	exemplarLabels?: LabelValues<T>;
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L239)</sub>

#### `ObserveDataWithExemplar`

```ts
interface ObserveDataWithExemplar<T extends string> {
	value: number;
	labels?: LabelValues<T>;
	exemplarLabels?: LabelValues<T>;
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L245)</sub>

#### `GaugeConfiguration`

```ts
interface GaugeConfiguration<T extends string>
	extends MetricConfiguration<T> {
	collect?: CollectFunction<Gauge<T>>;
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L326)</sub>

#### `HistogramConfiguration`

```ts
interface HistogramConfiguration<T extends string>
	extends MetricConfiguration<T> {
	buckets?: number[];
	collect?: CollectFunction<Histogram<T>>;
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L466)</sub>

#### `SummaryConfiguration`

```ts
interface SummaryConfiguration<T extends string>
	extends MetricConfiguration<T> {
	percentiles?: number[];
	maxAgeSeconds?: number;
	ageBuckets?: number;
	pruneAgedBuckets?: boolean;
	compressCount?: number;
	collect?: CollectFunction<Summary<T>>;
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L589)</sub>

#### `DefaultMetricsCollectorConfiguration`

```ts
interface DefaultMetricsCollectorConfiguration<
	T extends RegistryContentType,
> {
	register?: Registry<T>;
	prefix?: string;
	gcDurationBuckets?: number[];
	eventLoopMonitoringPrecision?: number;
	labels?: object;
}
```

<sub>[Source](https://github.com/holepunchto/bare-prom-client/blob/v15.1.9/index.d.ts#L768)</sub>

## See also

- [Bare modules](/reference/modules/bare-modules) — the full `bare-*` catalog.
- [Bare runtime API](/reference/bare/runtime) — the runtime these modules extend.
