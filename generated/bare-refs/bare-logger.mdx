...
#### `Log.format(...data: unknown[]): string`
...
#### `CompositeLog.clear(): void`

Clears the log output. A no-op on the base `Log`.

#### `CompositeLog.colors: boolean`

Whether this logger applies ANSI color styling to formatted output. Read-only.

#### `CompositeLog.debug(...data: unknown[]): void`
...
#### `CompositeLog.error(...data: unknown[]): void`
...
#### `CompositeLog.fatal(...data: unknown[]): void`
...
#### `CompositeLog.format(...data: unknown[]): string`
...
| Parameter | Type | Default | Description |
| --- | --- | --- | --- |
| `data` | `unknown[]` | — | Values to format; the first may be a `printf`-style format string (for example `%s`, `%d`, `%o`) with the remaining values as substitutions. |

#### `CompositeLog.info(...data: unknown[]): void`
...
#### `CompositeLog.warn(...data: unknown[]): void`
